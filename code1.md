...
| 源码状态 | **完整实现 + 七层优化，不是骨架**：tiling 整行条件修正 + 反向双缓冲（§9）→ 前向 `PipeBarrier<PIPE_ALL>`（§11.10）→ host 少开核定则（§14.3）→ 前向去 `TQue` 改两块 `TBuf` 轮转（A2，§16）→ 前向 barrier 按块摊薄 B=2/环 2B + ≥12KB 门（R10，§18）→ **整行小 tile 攒 4 块（BS 由 `2·BS·tile ≤ ub_size/4` 反推）+ 少开核拐点分向前向 8KB/核（R12，§19.4）** |
| 真机 | ✅ **7 轮 89 条 + 规则版 45 条×2 每轮 `ALL PASS fail=0 err=0`**，前后向**位级精确**（`fwd-fp16-large 0/469762048`）（§11.10.3 / §14.4）；⚠️ 2026-09-21 起发现**旧 `prof/prof2` 的 PASS 从不比数值**，已修（§17.1），修后 `base` 在 c4 上仍 `mismatch=0` ⇒ 采纳版结论不受影响。R10 采纳版在**提交树**再过一轮 45 条 + 8 条 `timed=1 mismatch=0`（§18.4），**R12 采纳版同一套门禁在提交树再来一次：45 条 ×2 全绿 + 8/8 `timed=1 mismatch=0 PASS`、`gate FAIL=0`**（§19.6），且提升前后逐字节同源校验（`.cand` == 提交文件，`.o` 与 A/B 里那个同号） |
| 仿真机 | ✅ 三轮全绿（quick 29 条 + 5 组全量 + 边界组）；⚠️ **对前向流水竞争无保护力**（串行执行），`bwd-fp16-BND-D32768-m2` 是**仿真假失败** ⇒ **正确性判据只认真机**（§8.3） |
| 比赛平台 | ✅✅✅✅✅ **五次提交均 `状态: Pass`、8/8 用例、每条 `precision_ratio: 1`**（首交 22:15 / 规则版 23:41 / A1+A2 02:09 / R10 05:25 / **R12 10:16，id `6ab091bdb0477ec41eeefdbf`**；§13 / §14.7 / §16 / §18.5 / §19.6）。⚠️ 榜单 `score=0` **不是我方慢**：平台自 2026-09-03 起对**所有人**停写分数（§13.3）。⚠️ 平台 µs 档读数与真机**三次同向反例**（本轮 −3.0%~+3.2% 无方向散布 vs 真机 c0/c2 −10%），且 **case1 三连涨 3.52→3.74→3.82 而本地 c0 同期在降** ⇒ "平台 case1 = 我方 fwd-small"这个映射假设本身可疑（§19.6 读法 2），已判**不再花提交位去追** |
| 性能 | ⭐ **中大档已贴带宽墙**：fp16-large 前向 **1089µs = 970 GB/s（910B 规格的 94.7%）**、反向 **1087µs = 972 GB/s（94.9%）**（§18.6；R10 在前向净取 **−2.2%~−2.5%**，四轮九模态同向）。小档经 §14 定则 **case1 −24% / case5 −11%**、经 §16 的 A2 **再 −7.9%**、经 **§19.5 R12 再 −10%（c0/c2 同形双 dtype 3.0→2.7，三轮中位数、散布 ≤0.1µs）** ⇒ 相对 §14.2 那个 blk=40 的起点（4.1~4.5µs）**累计约 −1/3**。⭐ **残余差距已量到"地板"以下**：空 kernel 实测固定 **1.2µs + 每核约 75~90ns 派发**（§19.2），即 blk=16 时**什么都不算也要 2.1µs**，而榜首 case1 是 1.42µs、平台 `best_time` 1.36µs ⇒ **剩下的不是我们代码里的时间**，是口径/形状差（§19 结题）。对平台固定基准 `best_time`：中大档 6 条 1–4%、两条反超榜首（§14.7）。⚠️ 平台侧本轮仍未复现真机位移（四条 1ms 用例 −0.31%~+0.57%、µs 档 −3.0%~+3.2% 无方向）⇒ 真机是性能判据、平台只判过不过（§18.5 / §19.6） |
| 已关闭路线 | ⛔ **分块+跨步 DMA**（`splitMode=3`，§14.5）；⛔ **arch22 源冻结广播编码**（§14.9）；⛔ **前向用手工事件对替掉 `PIPE_ALL`**（R5~R9，§17：`MTE2_MTE3` 不产生序、`MTE3_MTE2` 产生、窄 barrier 能编译但不管序）；⛔ **前向大档继续加批次**（§18.6：收益按 1/B 收敛到 ≤1.2%，且要放宽 host 的 `ub_size/4` 预算 ⇒ 破坏 A/B 可比性）；⛔ **用提交位试探平台用例形状**（§18.5：计分冻结，回报为 0）；⛔ **"每道 `PIPE_ALL` ≈0.28µs"这条外推**（§19.1 实测：4 道 barrier **全删**只值 0.45µs ⇒ 单价 ~0.11µs，小档 barrier 路线已到顶）；⛔ **"每核恰好一道 barrier 是最优"**（§19.3：blk16+BS=4 是 1 道/核读 2.8，blk8+BS=4 是 2 道读 2.6 ⇒ 省下的派发比那道 barrier 贵）；⛔ **BS=4 放开到 medium/多 tile 形状**（§19.2：8KB tile 吃 8 格环 = 64KB，正好吃满 host 给前向的全部预算，实测 13.8 vs 12.9 两轮同向劣化，已被 6144B 字节门挡死） |
| 唯一未定档项 | §9 两项优化**各自的加速比**未回退重测（§4.3）；平台 8 条用例的**真实形状**未拿到（§18.5 判定不再追；§19.6 记了"case1 三连涨"这个新旁证，同样不追） |
...
**一句话**：题 1 的**正确性**（真机 89 条 + 45 条全组矩阵多轮（§14.4 / §18.4 / §19.6 各 2 轮全绿）+ 平台 8/8×5）与**性能**（中大档双向都到 910B 规格的 94.7%/94.9%，小档前向累计约 −1/3）双双收口；前向流水的"最后一口"已经**判死在 API 语义上**（§17）。⚠️ §18 当时写的"没有待动项"是**过早收口**——§19 又找到 −10%，教训是"某条路线到顶"不等于"这一档到顶"（当时漏的是核数与批大小的联合维度）。现在的收口是**有地板线的**：空 kernel 实测固定 1.2µs + 75~90ns/核 ⇒ blk=16 什么都不算也要 2.1µs，而榜首 1.42µs（§19.2）⇒ **小档剩余差距不在我方代码里**，除非拿到平台的用例形状或 `best_time` 口径（§18.5 已判定不花提交位去追）。
...
```
4805684cb99ca7bd4856209e59b584e3  op_kernel/mhc_expand.cpp             14,061B  ← ⭐在册内核 = 第五次提交版 `74282cfa…` **仅注释不同**（6 行全 `//`，kernel `.o` 复编后 `e0cfb94949e5`+`91e603334164` 一字未变，§19.6）
61543c1b17f94cc7c99c4bf4178fc197  op_host/mhc_expand.cpp               10,831B  ← ⭐同上，`0c83f8d9…` 的注释修订版（2 行 `//`）
...
- **host**：`a16c371d…`（首交，tiling 条件修正版）→ `c76d30be…`（第二次提交＝§14.3 少开核定则）→ **`3c2dadc8…`**（第四次提交＝§18 给 `ub_size/4` 补"环深上限"那条注释）→ **`0c83f8d9…`**（第五次＝`min_io_per_core` 按方向取值 + 注释整段换成 R11 实测）。
- **kernel**：`d9af5611…`（原始）→ `0fb9e6ec…`（§9.2 反向双缓冲）→ `53ee60e5…`（§11.10 前向 barrier）→ `daf2b8ed…`（第三次＝§16 A1+A2 去 `TQue`）→ `038d65a0…`（第四次＝§18 R10 barrier 按块摊薄 B=2）→ **`74282cfa…`**（第五次＝§19.4 R12 三条支路 `BS∈{1,2,4}` + 8 格环）。
- **sha256**（`--dry-run` 回传口径，§3.4 / §14.7 / §18.5）：`tiling_h be5f660b02fef22a…` · `tiling_key_h b79c2cbda239ff8d…`（两份至今一字未动）· `kernel_cpp` `a1744d14…`(9,318B/二交) → `1f240978…`(9,829B/三交) → `ab8654dc…`(12,187B/四交) → **`bab9e6ef8ad1ce39…`(13,925B/五交)** · `host_cpp` `32cce2e9…`(9,143B) → `278b7b84…`(10,343B/二交) → `3b3fa433…`(10,409B/四交) → **`ad2895cb38703cba…`(10,799B/五交)**。
...
npu_debug/test_mhc_expand_npu.cpp   2e8cc54c245f600a27436f37e8ff1322  ← 演变：c7a3e698… → 21:10 改 IEEE inf → 21:25 加 prof 组 → 23:2x 同步少开核定则注释 → §19.6 把 `core_cap` 的分向门限同步进 mirror（**只影响 harness 预测打印，不影响判据**）
...
| `op_host` 少开核规则 | `min_io_per_core = backward ? 6144 : 8192` | §19.3：前向在 BS=4 之后谷底左移到 8KB/核（96KB/12），反向仍在 6KB/核。注释里的实测数字整段换成 R11 的 `base` 列 |
...
出分后回读两份源码，发现新写的两条 BS 界**都是错的**：① 把 host 预算写成"`ub_size/4`=48KB"——48KB 是 `ub_size==0` 的兜底路径（`op_host` 里 `ub_size=192*1024`），真机 `ub_size`=256KB ⇒ 预算 **64KB**（§11.8 已用 D=32768 走整行钉死过这件事）；② 把 BS=2 的下界 `tile>=12288` 写成了"tile<=24575B 开 BS=2"，凭空造了一个不存在的上界。改注释不动代码，kernel `4805684c…`(14,061B)、host `61543c1b…`(10,831B)，两棵树同步推齐后重构建：
