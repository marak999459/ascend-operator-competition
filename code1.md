...
| 源码状态 | **完整实现 + 五层优化，不是骨架**：tiling 整行条件修正 + 反向双缓冲（§9）→ 前向 `PipeBarrier<PIPE_ALL>`（§11.10）→ host 少开核定则（§14.3）→ 前向去 `TQue` 改两块 `TBuf` 轮转（A2，§16） |
| 真机 | ✅ **7 轮 89 条 + 规则版 45 条×2 每轮 `ALL PASS fail=0 err=0`**，前后向**位级精确**（`fwd-fp16-large 0/469762048`）（§11.10.3 / §14.4）；⚠️ 2026-09-21 起发现**旧 `prof/prof2` 的 PASS 从不比数值**，已修（§17.1），修后 `base` 在 c4 上仍 `mismatch=0` ⇒ 采纳版结论不受影响 |
| 仿真机 | ✅ 三轮全绿（quick 29 条 + 5 组全量 + 边界组）；⚠️ **对前向流水竞争无保护力**（串行执行），`bwd-fp16-BND-D32768-m2` 是**仿真假失败** ⇒ **正确性判据只认真机**（§8.3） |
| 比赛平台 | ✅✅✅ **三次提交均 `状态: Pass`、8/8 用例、每条 `precision_ratio: 1`**（首交 22:15 / 规则版 23:41 / A1+A2 02:09；§13 / §14.7 / §16）。⚠️ 榜单 `score=0` **不是我方慢**：平台自 2026-09-03 起对**所有人**停写分数（§13.3） |
| 性能 | fp16-large 前向 **1117.3µs / 946GB/s**、反向 **1088.3µs / 971GB/s**（§11.11）；小档经 §14 定则 **case1 −24% / case5 −11%**，再经 §16 的 A2 **case1 再 −7.9%**。中大档 6 条已贴平台基准 1–4%（两条反超榜首），**残余缺口 = 小档每任务固定开销 ~1.7µs，投入产出比极低**（§14.8） |
| 已关闭路线 | ⛔ **分块+跨步 DMA**（`splitMode=3`，§14.5）；⛔ **arch22 源冻结广播编码**（§14.9）；⛔ **前向用手工事件对替掉 `PIPE_ALL`**（R5~R9，§17：`MTE2_MTE3` 不产生序、`MTE3_MTE2` 产生、窄 barrier 能编译但不管序 ⇒ 跨任务 MTE2/MTE3 重叠在这条纯搬运路径上拿不到） |
...
**时间线（2026-09-21 凌晨）**：P1 探针定 arch22 DMA 口径（§14.9）→ A1/A2 采纳 + **第三次提交 case1 再 −7.9%**（§16）→ 修两个判据级缺陷（计时档不比数值、pkg 会跑旧核）→ **R5~R9 事件对路线以负结果结案**（§17）。

**一句话**：题 1 的**正确性**（真机 89+90 条 + 平台 8/8×3）与**性能**（中大档贴基准、小档累计压掉 30%）双双收口；前向流水的"最后一口"已经**判死在 API 语义上**（§17），只剩"barrier 按块摊薄"这一个可量化未试项和"§9 加速比未定量"这条尾巴。
