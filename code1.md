...
| 目标芯片 | `ascend910b` —— **已实测坐实**：真机 `02aeb` = **Ascend 910B3，NPU ID=7**（旧记录 ID=2 作废），驱动 25.5.0，CANN 9.0.0，HBM 64GB（§11.8）。⚠️ **SoC 标签随机器变，不是随源码变**：题3 那台容器（`bd23984d…`，同为 910B3、驱动 25.5.5）上纯 `ascend910b` 编出的包**装载即 161001 `Get regInfo failed`**，必须在**远端副本**上补 `ascend910_93`（`CMakeLists.txt:7` + host `:202` 的 `.AddConfig`，提交文件不动）才跑得起来 ⇒ §11.8 那句"`ascend910b` 口径正确"只对 `02aeb` 成立（§11.10） |
| 源码状态 | 【实测】`op_host/mhc_expand.cpp` / `op_kernel/mhc_expand.cpp` 已含**完整实现 + 性能优化 + 前向补序**（tiling 整行条件修正 + 反向双缓冲流水线，§9；前向 `PipeBarrier<PIPE_ALL>`，§11.10），**不是**骨架 |
| 真机验证 | ✅ **当前提交源已全量复验通过**（2026-09-24 18:55–18:56，`bd23984d…` / 910B3 / 50 AIV / UB 64KB，**6 组 64 条 = 前向 19 + 反向 45，0 FAIL**，逐位精确；原始日志 `code1/npu_debug/logs/mhc1_npu_recheck_20260924.txt`，§11.10）。⭐ 9/20 那轮 **前向 14/14 全 FAIL** 的根因（`ForwardOneBlock` 纯搬运 ⇒ `TQue<VECIN>` 的序不约束 MTE1 读 / MTE2 写，§11.9.4）**已被坐实修掉**：修法 = 三候选里的 **(c) `PipeBarrier<PIPE_ALL>()`**（`op_kernel/mhc_expand.cpp:115`，紧跟 `DeQue<DT_X>()`），随 `eaf0d4e`（9/22 的**题3** 提交）一并落进源码、**当时既没跑真机也没写文档** ⇒ 本轮是那次改动的第一次真机读数。同一条提交里还重排了**反向双缓冲**，本轮顺带复验 = 反向 45 条全绿（含 `bwd-*-large` 0/58720256）。`bwd-fp16-sat-m2` 的 fp16 溢出分歧也已闭：启动器已按"真机 `Cast(CAST_RINT)` 给 inf"为准（`test_mhc_expand_npu.cpp:121-126`），本轮 0/512 PASS。msprof 仍未采 |
...
| 比赛平台提交 | 无记录（未提交过） |
| 性能 | ⚠️ **仅有真机 kernel 墙钟粗测**（启动器 sync 前后计时，分辨率 1ms）：反向 `large` 8192×7168×8（读 939.5MB + 写 117.4MB ≈ 1.06GB）→ `kern=0.001s`，含 `medium` 也一律印 0.001s ⇒ 单靠它**不足以支撑 §9 两项优化的 A/B 结论**。正式基线仍待 `msprof`（§11.6），⚠️ **9/24 复验仍走同一 1ms 分辨率启动器 ⇒ §9 双缓冲那笔账至今没结**；"等前向修完再一次性采"这个前提本轮已经满足。 |

**一句话**：当前提交源（kernel md5 `53ee60e5…`）**真机 6 组 64 条全绿、逐位精确**（2026-09-24 复验，§11.10）—— 9/20 那轮"反向全绿、前向 14/14 全红"的分歧已经闭掉，根因（`ForwardOneBlock` 缺 MTE2→MTE1 序，§11.9.4）由候选 (c) 的 `PipeBarrier<PIPE_ALL>` 修掉，**那次改动是 9/22 随题3 提交 `eaf0d4e` 落进源码、当时既没跑真机也没写文档，本轮才补上读数**。仿真侧 5 组已收口（§9.5.2）且已证明**仿真对前向无保护力** ⇒ 判据以真机为准。剩余未知项只有两件：① 性能基线（msprof 未采，§1 性能行；前向已绿 ⇒ 无阻塞）；② **平台侧无提交记录**（§1『比赛平台提交』格），交不交由用户定（§0 约束：改代码/提交不自动动手）。
...
| md5 基线（2026-09-24 核对） | 完整值见 §6.1（现值：host `a16c371d…` · kernel `53ee60e5…` · tiling.h `f759a052…` · tiling_key `267e0125…`）。**这份字节 = 9/24 真机 64 条全绿的输入**：远端副本 `~/mhc1_check/code1` 上 kernel/tiling.h/tiling_key 三枚逐字节相同，host 差**且仅差**那一行 SoC 双注册（把它 sed 回去后 md5 正好回到 `a16c371d…`，§11.10）。后续判断"文件是否被动过"以此为准 |
...
- ~~❌ 从未上机跑过 → 数值正确性、对齐行为、UB 容量全部未验证~~ → **已验证并出结论**（§11.9）：反向 24 条位级精确 PASS；**非对齐 D=7167 / D=100 / D=1 全过**；**UB=256KB ⇒ 64KB 预算口径被真机反证成立**（D=32768 整行 vs 32769 退化 tile=2048，与 §9.1 推导吻合）。⭐ 那条由此新增的未排除项（**前向真机 14/14 全 FAIL**，§11.9.4）**已排除**：修法落地后全量矩阵重跑 = 6 组 64 条全 PASS（§11.10，9/24）
...
- ❌ 比赛平台精度判据未知（`allclose(rtol=1e-3)` 还是**逐 bit**）—— ⚠️ 本轮新增一个必须靠它来裁的具体分叉：**fp16 溢出取 `inf` 还是 `65504`**（§11.9.5；9/24 已在启动器侧按 `inf` 自洽，§11.9.6 #2 —— 仍只有平台能裁）
...
- **实测结果**：编译门 ✅ 首过（§11.8）；运行 **不满足"全 PASS"** —— **反向 24/24 位级精确 PASS、前向 14/14 全 FAIL**（§11.9.3），错误率/形状已逐条记录 ⇒ **当时**判"不可交"。⭐ **9/24 复验已把它翻过来**：同一套 harness、当前提交源，6 组 64 条全 PASS（§11.10）⇒ 按 §5 判据本题**已可交**，剩下的只是"交不交"这一步归用户。
...
### 6.1 md5 基线（2026-09-20 记录，**2026-09-24 真机复验时更新**：kernel 与两份 npu_debug 脚本已换值，其余未动）

```
a16c371d1bb39a81d0efecd24db71b49  code1/op_host/mhc_expand.cpp        ← 优化后（tiling 条件修正）
53ee60e532ee1730b3bd0f1bc713c983  code1/op_kernel/mhc_expand.cpp      ← **2026-09-24 核对**：前向 PipeBarrier + 反向双缓冲重排（随 eaf0d4e 落进、当时未记档；本轮真机 64 条复验通过，§11.10）
...
66f63fcd660d92c83cf9c3a2c7e00010  code1/npu_debug/test_mhc_expand_npu.cpp  ← 真机 ACL 启动器（§11.9.1）：**dlopen 取 aclnn 入口，不做链接期依赖**；**9/24 现值**：相对 `c7a3e698…` 的差量 = fp16 溢出参考改按真机 `Cast` 语义给 inf（§11.9.5 那条分歧因此闭）+ 组名白名单/计时
221777360927eefe1084a2a9ceb5a34d  code1/npu_debug/build_npu.sh             ← 探测 set_env + 时间戳/`nm -D` 防假成功 + 链接 `-lnnopbase -lascendcl -ldl`（⚠️ **不含 SoC sed**，跨机器要先按 §11.10 补 `ascend910_93`）
...
| `ASCEND_COMPUTE_UNIT` | 在 `02aeb` 这台机上 **`ascend910b` 口径可用**，提交源四文件零改动（推送后 md5 逐字节比对 = 当期基线）。⚠️ **这条机器相关、不普适**：9/24 换到题3 那台容器，纯 `ascend910b` 装载即 161001，必须远端副本补 `ascend910_93`（§11.10） |
...
> **反向 24 条全 PASS（含逐位精确），前向 14 条全 FAIL，无一例外。** 提交源四文件 md5 仍 = **当期** §6.1 基线（那一轮零改动）。⚠️ 这份 kernel（`0fb9e6ec…`）已不是仓库现值：9/22 `eaf0d4e` 换成了 `53ee60e5…`，其真机读数在 §11.10。
...
- **fp16 溢出语义分歧（确定性，非竞争）**：`bwd-fp16-sat-m2`（输入 32768，m=2 ⇒ 65536 溢出）真机 `Cast(CAST_RINT)` 给 **+inf**（512/512 全错），CPU 仿真与本地参考给 **65504（饱和）**。同组 `sat-m1`（不溢出）PASS ⇒ 只有溢出分支分歧。**后续（9/24）**：按「参考改成 inf」收口 —— 启动器 `test_mhc_expand_npu.cpp:121-126` 已如此，`sat-m1/m2` 各 0/512 PASS；**平台口径仍未排**（§11.9.6 #2）。

#### 11.9.6 三个决策点的现状（2026-09-24 更新；9/20 那份「待用户拍板」原表在 git 历史里）

| # | 决策点 | 现状 |
|---|---|---|
| 1 | **前向竞争怎么修** | 候选 **(c) 已落地**：`op_kernel/mhc_expand.cpp:113-115`（DeQue 之后一条 `PipeBarrier<PIPE_ALL>()`），**但它是 9/22 随题3 提交 `eaf0d4e` 一起进去的、当时既没跑真机也没记档**（同一条提交还重排了反向双缓冲）⇒ 本轮 §11.10 才是它的第一次真机读数。⚠️ 当时写在候选里的那句代价（"把 m 次写出串行化，大 S 下带宽利用率有代价"）**至今没量**：修法生效后前向已可用，要不要换 (a) 的 `SetFlag/WaitFlag` 配对是一笔**性能账**，归 §11.6 msprof，不属正确性账 |
| 2 | fp16 溢出：改**核**还是改**参考** | **按参考收**：启动器 `npu_debug/test_mhc_expand_npu.cpp:121-126` 已把期望改成 inf（"真机 `Cast(CAST_RINT)` 超量程按 IEEE 给 inf"），9/24 `bwd-fp16-sat-m1/m2` 各 0/512 PASS ⇒ 本机两侧一致。**仍未排**的是比赛平台口径（§4.3 那条老未知项），只有平台的判据能裁它 |
| 3 | harness 未知 mode 静默通过 | **仍待办**（属改代码，按 §0 约束不动手；本轮 6 组 64 条的计数已按组名白名单核对过，未再撞到拼错组名） |

**性能基线（§11.6 msprof）**：9/20 写的"等前向修完再一次性出 A/B"这个前提**已满足** —— 剩下的只是开跑，不再是"数字无意义"。

### 11.10 ⭐ 当前提交源的真机全量复验（2026-09-24 18:55–18:56，`bd23984d…` / 910B3 / 50 AIV / UB 64KB）

**为什么要有这一节**：§11.9 的读数属于 `0fb9e6ec…` 那份 kernel，而仓库里现在这份（`53ee60e5…`，含前向 barrier + 反向双缓冲重排）**自 9/22 落进源码后一次都没上机** ⇒ "代码看似完成、真机从未验证"这条老状态在文档和 AGENT.MD 里挂了两天，是错的。

**流程**：本地 `code1/` 整目录 tar 管道推到 `~/mhc1_check/code1`（⛔ 不碰题3 的 `~/sfa_real`）→ 远端副本补 SoC → `bash npu_debug/build_npu.sh` → `bash npu_debug/run_npu.sh <组> 50 64` 逐组跑。

| 判据 | 实测 |
|---|---|
| 精度 | **6 组 64 条 = 前向 19 + 反向 45，`=== ALL PASS ===`、0 FAIL**，全部 `maxdiff=0.00000 mismatch=0/N`（逐位精确）。含 `large` 前向 **0/469762048**、反向 0/58720256，含全部 UB 预算边界（D=32768 整行 / 32769 退化 / 24576/24577 / 26000/26001） |
| 前向是否真修好 | ✅ §11.9.3 那张全红表里的**每一类同参用例**（`S1D1`/`small`/`medium`×2 dtype/`large`×2 dtype/`MTL-D70000-m2`/`m1`/`m16`/`ROW-m8`/`D7167`/`D100`/`BND-D32769`/`UB48-D26000`）今天全部重跑并绿，含窗口最宽的 `ROW-m8` 与 `large` ⇒ §11.9.4 的机理判定与修法同时得到反证支持。⚠️ 用例集比 9/20 更大（现 64 条 vs 当时 38 条，harness 已换值），不是逐条同名单 |
| 双 dtype | fp16 + bf16 各自覆盖，无单边失败 |
| 提交源 | kernel `53ee60e5…` / tiling.h `f759a052…` / tiling_key `267e0125…` 三枚远端=本地；host 远端 `1eab0069…` 只差 SoC 那一行（sed 回去 = `a16c371d…`） |

**🔴 本轮踩到的机器差异（下次换容器直接照抄）**：这台 910B3 上纯 `ascend910b` 编出来的包**装载即 161001**，且 `run_npu.sh` 只报 `GetWorkspaceSize failed st=161001`（25/25 全"错误"，看着像算子缺陷）。真因要开 `ASCEND_GLOBAL_LOG_LEVEL=1 ASCEND_SLOG_PRINT_TO_STDOUT=1` 才看得见：`errno[161001] Get regInfo failed, The binary_info_…` + `Check nnopExecutor != nullptr failed`。⇒ 修法与题3 同源：**只在远端副本**打 sed ——`CMakeLists.txt:7` 换 `set(ASCEND_COMPUTE_UNIT ascend910_93)`、`op_host/mhc_expand.cpp:202` 换 `.AddConfig("ascend910b").AddConfig("ascend910_93")`，**两处各 `grep -c` 命中 1 才算打上**（跨题纪律，code3.md §7 #2：远端旋钮必须数命中 —— 否则「旋钮没打上」会伪装成「算子有 bug」）。⚠️ 由此 §11.8 那句"`ascend910b` 口径正确"订正为：**它只对 `02aeb` 那台成立**，SoC 标签是机器属性不是源码属性。

**证据**：`code1/npu_debug/logs/mhc1_npu_recheck_20260924.txt`（64 条逐行 + 各组 `ALL PASS` 汇总）、`mhc1_npu_161001_before_soc_sed_20260924.txt`（补 sed 前的失败原文）。两份都已从易失容器 tar 回本地入库（`.log` 被 `.gitignore` 吃，故落 `.txt`）。

**本轮零代码改动、零提交**：动的只有文档与 `npu_debug/logs/` 两份证据。
