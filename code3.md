...
- **(d) 机制归属**：能同时解释"六点齐平 3.7× + 形态斜率正常"的只有**算力类别**：§15.62 段账里 score 51~55 % + PV 31~34 % ⇒ 我们 82~88 % 的时间在 AIV 上做 fp32 乘加，而榜首 `big1` 量级只要 2.18~3.76 就必须把这两段搬上 Cube（§15.65 已证平台 MIX 门开着）。⇒ **题 3 的差距不是"哪里写坏了"，而是"哪一段还没上 Cube"**，与 §15.66 的重排一致（M1 是唯一有量级的线，但它值 40~60 分而不是"回到 22.19 之上一点点"）。
