...
> 📍 **坐标（2026-09-22）**：路线已被 §14.5 的表取代（§11.3 作废）；**§15 已把 P0.5 在本地判到底** —— 修正是**可证零回归**的（15 个 fp16 格选出的 `(nb,n_blk)` 一字不变），fp32 超预算那格**平台走不到**（§15.5 可达性收窄），而 P1/P3 的真实约束换成了"**`n_blk` 天花板 64、要拿它必须把 `nb` 从 32 降到 8**"（§15.4）。⇒ 下一步要么落 P0.5 的笔（方案 A，§15.7），要么直接从 **P1①（逐元素填 UB → `DataCopyPad`）** 开刀；两者都要先过 §13.6 编译门，**真机那一步动手前问用户**。
...
> ⚠️ **2026-09-22 §15 对本条的两处修正**：① "搬运单位 = 16" **不是物理约束，是 `CalcBlocking` 打分优先大 `nb` 的产物** —— 真实预算下 `n_blk` 天花板是 **64**，代价是 `nb` 32→8（§15.4）；② 上面"未验证部分"里那条"若平台 fp32 用例存在且现在 6/6 是 PASS，那更可能是静默共享地址" **可达性已收窄**：OpDef 只登记 `{DT_FLOAT16, DT_FLOAT}`、bf16 由实验 1 排除、题面只允许 fp16/bf16 ⇒ fp32 那格是**我方自己多开的门**（§15.5），风险等级下调一档，但 P0.5 **不撤销**（理由换成 §15.4 第 3 条）。
...
| **P1 搬运聚合** | ① `our:352-361` 逐元素填 UB → `DataCopyPad`，**搬运单位 = `min(sbs, n_blk) = 16` token**（不是官方 32；⚠️ §15.4 修正：**16 是 `CalcBlocking` 优先大 `nb` 的打分产物，不是物理约束**，真实天花板 **64**、代价是 `nb` 32→8），可仿 `vec:921` 用 `srcStride` 两块一指令、异常回落两条；② bf16 分支改成"原始 `uint16` 整段拷进 UB + 向量化 `Cast`"，消灭 `our:414/419/450` 的 GM 重读；③ "遇 -1 即停"换成"补零 + 记有效长度 + 事后掩 `-2e38`"（`vec:1041/1073/486-499`） | 三条被官方原文钉实；**③从"可选"升为前置**（不做③就没有定长窗口，也就没有 ping-pong） | §14.2 |
...
| **P3 score 向量化** | `sbs` 内 token 已连续 ⇒ K 整段进 UB 后按 `[j][d]` 布局做 `Mul` + `WholeReduceSum`（带 stride 版已由编译门裁定可用）；转置缓冲在 `n_blk=16` 下约 `16×576×4 ≈ 36 KB`，**但现行 `(nb=32, n_blk=16)` 只剩 7,890 B 余量、装不下；`(16,32)` 有 42,962 B 才装得下 ⇒ 真实前置是"敢不敢降 `nb`"，不是"做没做 P0.5"**（§15.4 第 2 条） | UB 冲突从"警告"变成"可算的数" | §13.10、§13.14 |
...
## 15. ⭐ 2026-09-22：P0.5 离线收口 —— 把"该修"变成"修了会动谁"，并顺手量出 `n_blk` 的真实天花板 64

起因：题 1 两条轴（核数轴、H3）都已封存，平台侧当前可读钱为 0；题 3 是三条题里唯一还活着的方向。§14.5 把 P0.5 排在 P1 前面，但 §13.14 只给了一行结论 + 三行数字，**没有回答"修完之后 fp16 那条通过路径会不会被带动"** —— 而这恰好是**不上真机就能判死**的部分。本轮全部是纯算术（BigInt 复刻），`code 3/code/` 四个提交文件**一行未改**。

### 15.1 方法：把两个算式逐项抄进一个可复算的脚本

`code 3/cpu_debug/ub_budget_check.js`（本地 Node，非提交面、不上机）：

- `hostNeed()` = 照抄 `op_host:39-49`（K/V/kr 写死 `2ULL`、无 `lseBuf_`）；
- `trueNeed()` = 照抄 `op_kernel:168-177` 十次 `InitBuffer`（K/V/kr 按 `sizeof(DT_QUERY)`，多一格 `lseBuf_ = 2*nb*4`）；
- `choose()` = 照抄 `CalcBlocking`（约束 `nb*n_blk >= sbs`、门槛 `CalcUbNeed <= ubSafe`、打分 `score = nb*100000 + n_blk` ⇒ 优先大 `nb` 再大 `n_blk`），三条都在 `op_host:63-72` 原文里核过。
- ✅ **缓冲项清单已穷尽**：kernel 全文只有 **10 个 `InitBuffer` / 10 个 `TBuf`**（`:460` 声明行），与预算式逐项对齐后，两边差异**恰好两处** —— ① K/V/kr 的字节宽写死，② `lseBuf_` 缺项。没有第三处，所以"修 P0.5"的工作面是可枚举的。
- `ubSafe = 196608/100*95 = 186,770`（`op_host:32-33` 常量）。

### 15.2 §13.14 那三行数字逐字节复现（可复核，不是转述）

| 口径（bench 形状 `B=1,S1=128,S2=8192,N1=8,D=512,Dr=64`，选中 `nb=32, n_blk=16`） | 字节 | 对 196,608 物理 UB |
|---|---|---|
| host 估算（现行 `CalcUbNeed`） | 178,624 | 通过 ⇒ 所以 tiling 不会降级 |
| kernel 实际 **fp16**（=2B） | 178,880 | 通过（差 256 B = `nb*8` 的 `lseBuf_`） |
| kernel 实际 **fp32**（=4B） | **213,696** | ⚠️ **超 17,088 B**，与 §13.14 同数 |

### 15.3 ⭐ 新结论（本轮最有用的一条）：**修正在 fp16 上零回归，可证**

15 个 fp16 格（`Q_D ∈ {512,128,64} × sbs ∈ {1,16,32,64,128}`）逐个跑"现行式选谁 / 修后式选谁"：**每一格选出的 `(nb, n_blk)` 都完全相同**（`Q_D=512` 恒为 `32/16`，`128/64` 恒为 `32/128`），差异只有"占用数字 +256 B"。

⇒ **P0.5 不是救火，是把尺子修准**：它**不可能**动到 §5.8.4 那支 6/6 通过路径的分块决策（那支走的就是 fp16 实例）。这一条在落笔前值很多钱 —— 它把"要不要冒改提交树的风险"这个决策，换成了"改完只需复验 fp32 那几格"。

### 15.4 新量：`n_blk` 的真实天花板 = **64**，且拿到它必须把 `nb` 从 32 降到 8

按**真实占用 ≤ ubSafe** 反解（`Q_D=512, Dr=64`，fp16）：

| `nb` | 候选里最大 `n_blk` | 连续上界 | 真实占用 | 余量 |
|---|---|---|---|---|
| 32（现行） | **16** | 19 | 178,880 | **7,890 B** |
| 16 | **32** | 50 | 143,808 | **42,962 B** |
| 8 | **64** | 67 | 178,592 | 8,178 B |
| 4 / 2 / 1 | 64 | 76 / 81 / 83 | 159,056 / 149,288 / 144,404 | 27,714 / 37,482 / 42,366 |

三条直接给 P1/P3 的读法：

1. ✅ `op_host:53-54` 的注释只论证了"`n_blk=128` 整块读不可能"（它算的是 K+V=262,144 B）。补全后同样硬：**128 token 的 K+V+kr 净 278,528 B > 196,608 B** ⇒ 天花板落在 **64**（连续上界最大 83，候选档位只有 64）。⇒ **P1 的"搬运单位"最好情形是 64 token/次，不是官方 `vec` 的 32 也不是 `sbs=128` 整块**；§14.5 P1 那句"受 §13.14 预算约束所以是 16"要改口径：**16 不是物理约束，是 `score` 优先大 `nb` 的产物**。
2. ✅ **P3 那格 ~36 KB 转置缓冲装不装得下，现在是有数可查的**：现行 `(32,16)` 只剩 **7,890 B** 余量 ⇒ **装不下**；`(16,32)` 有 **42,962 B** ⇒ **装得下**。⇒ P3 的真实前置不是"做没做 P0.5"，而是"**敢不敢把 `nb` 从 32 降下来**"，这是一次打分规则改动（`op_host:36` 的 `NB_CAND` 顺序 / `:70` 的 `score` 式），不是预算式改动。
3. ✅ **这才是 P0.5 排在 P1/P3 之前的定量理由**（§13.14 当时只写了"会把这个洞放大"）：同一张表按 fp32 实例算，天花板直接**减半**（`nb=32` 只能到 8、`nb=8` 只能到 32、`nb=1` 连续上界 41）。⇒ 拿一把按 fp16 写死的尺子去扩 `n_blk`，fp32 那扇门会从"已经超"变成"超更多"。

### 15.5 可达性收窄：**fp32 那一格不是平台会走的路** —— §13.14 的风险等级下调一档

四条来源核对（全部只读）：OpDef 六个张量登记的是 `.DataType({ge::DT_FLOAT16, ge::DT_FLOAT})`（`op_host:265-297`，**无 `DT_BF16`**）；`op_host:209` 只有 `DT_FLOAT → C_DT_FLOAT`，其余全落 `C_DT_FLOAT16`（bf16 靠 `:210` 的 `is_bf16` 走 2 字节实例 ⇒ **host 写死 2 字节对 fp16 与 bf16 都是对的**）；题面 `official_problem_statement.md:343-351` 只允许 float16/bfloat16；§4 实验 1 实测真 `ACL_BF16` 张量走 `GetWorkspaceSize` 直接 161002。

⇒ **fp32 超预算是我方自己多开的一扇门**（注册 + `op_kernel:504-506` 显式实例化都在，但平台没有理由走到）。风险从 §13.14 暗示的"**现有通过版里埋着正确性炸弹**"下调为"**死代码路径上的隐患**"。⛔ 但**不因此撤销 P0.5**：理由换成 15.4 第 3 条 —— 它服务的是"下一把尺子要量在活路上"，不是"救现在这条路"。

### 15.6 本地判不了的、以及上机要问的那一个问题

- ⚠️ `TBuf` 申请超过物理 UB 时是**分配失败（报错）**还是**静默越界（踩别的缓冲）**，纯算术判不了。若平台真发 fp32，前者会 WA 成"整 case 挂"、后者会 WA 成"高分散乱" —— 现在 6/6 通过 ⇒ **两种都没被观测到**，所以这条只能等真机或者干脆**把门关上**（见 15.7 方案 B）。
- ⚠️ 判分 case 的真实 dtype 仍然只有间接证据（题面 + 实验 1）；`refs/sfa/cases/` 那 10 个 `.bin` 文件名与内容头都不带 dtype 字符串。

### 15.7 落笔方案（两个，都还没做）

- **方案 A（补尺子）**：`CalcUbNeed` 增加 `dtype` 形参、K/V/kr 用 `sizeof` 的实例值、补 `lseBuf_` ⇒ 需同时给 `CalcBlocking` 加参（`op_host:58` 签名、`:181` 调用点），而 `DT_QUERY` 选择在 `:207-212`，**在 `CalcBlocking` 之后** ⇒ 要把 dtype 读取上移。收益：fp32 自动降级到安全分块（15.4 表），fp16 决策一字不变（15.3）。
- **方案 B（关门）**：从 OpDef 与 kernel 实例化里**摘掉 `DT_FLOAT`**，只留 fp16/bf16 语义 ⇒ 改动更小、把 §15.5 那扇门直接焊死，但会丢掉"框架万一以 fp32 喂"的兜底（而这兜底本来就是**超预算的**，等于现在留着也没用）。
- 我的倾向：**A 为主、B 不作**（A 是"修尺子"，B 是"改契约"，B 动的是与题面对齐的注册面，收益不比 A 多）。⚠️ 两者都要在**副本**上做、先过 §13.6 编译门、再真机跑 §5.9 语料；提交树 `code 3/code/` 是 §5.10.1 通过版 md5 锁定，动它之前按 §0 说清"改什么/为什么/怎么验"。

**本轮未动的东西**：`code 3/code/` 四个提交文件**一行未改**，磁盘态复验与 §5.10.1 通过版逐字节一致（kernel `bcb2f654c4e0`、host `fe3d1bc013c4`、tiling.h `6144697b725c`、tiling_key `02dd48f90480` 460B），调试输出 grep 仍为 0；新增的只有本地脚本 `code 3/cpu_debug/ub_budget_check.js`（含 15.4 的第二张表）。
