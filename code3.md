...
**(f) 状态**：远端已还原干净构建（`big1` 0.6577 ms、`超差 0/524288`、`P47 FORCE8|ABL` grep = 0/0、kernel md5 `0d5c5e28` = 提交态）；`code 3/code/` 四文件全程未动；未提交平台。§15.61 的 (b)(c)(d) 三处已就地订正。下一轮第一件事：按 §15.61(e) 清单**直接动手写 M1**（`SFA_CUBE` host 开关 gate、`Fixpipe` dst 落在 `outGm_[s1Base]` 的两段 2.5 KB、先锁 `golden_cube/`），**不再需要任何新的探针场次** —— #33 的"暂存预算判停"到此解除。
