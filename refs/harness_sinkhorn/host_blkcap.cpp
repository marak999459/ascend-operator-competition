...
namespace optiling {
    static ge::graphStatus TilingFunc(gert::TilingContext *context) {
        ...
        // Each dispatched block costs ~0.1-0.2us on 910B3 even when it has no matrix
        // to do, so small batches must not pay for all 40 cores.
        const uint32_t blocks = (static_cast<uint32_t>(batch) < static_cast<uint32_t>(num_cores_aiv))
                                    ? static_cast<uint32_t>(batch)
                                    : static_cast<uint32_t>(num_cores_aiv);
        tiling->batchPerCore = (static_cast<uint32_t>(batch) + blocks - 1u) / blocks;
