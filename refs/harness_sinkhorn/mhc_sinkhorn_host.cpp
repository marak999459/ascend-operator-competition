// Host侧Tiling实现（用于本地复现/验证的重建桩，逻辑与比赛工程一致）
...
namespace optiling {
    static ge::graphStatus TilingFunc(gert::TilingContext *context) {
        ...
        // 输入 logits：(T,n,n) 或 (B,S,n,n)；mask 为可选输入，忽略
        ...
        // 【关键】所有核统一的轮数 = 矩阵最少那个核的矩阵数。
        // 用于保证各核 SyncAll 次数一致，避免核间栅栏死锁。
        {
            const uint64_t bpc = tiling->batchPerCore;
            const uint64_t cores = static_cast<uint64_t>(num_cores_aiv);
            uint64_t max_chunks = bpc;
            if (bpc * (cores - 1u) < static_cast<uint64_t>(batch)) {
                max_chunks = static_cast<uint64_t>(batch) - bpc * (cores - 1u);
            } else {
                max_chunks = 0;
            }
            if (max_chunks == 0u) {
            ...
            tiling->maxChunksPerCore = static_cast<uint32_t>(max_chunks);
        }
