// Kernel: mHC-Sinkhorn (Sinkhorn-Knopp double stochastic), vectorized step-5
//   = step-4's instruction set, replayed over G_MAX=8 matrices inside one instruction.
//
// Why: 题2 is instruction-issue bound, not bandwidth bound (code2.md §11.6 / §11.14:
// fp32 1024x8 spends 78us of aiv_vec_time at ~1.3 GB/s). §11.10 and §11.11 both showed
// device time tracks instruction *count*, with data-per-instruction nearly free, so the
// lever is covering more matrices per instruction: 9 instructions per round become
// 9/G per matrix.
//
...
//
// GM<->UB: when the fp32 matrix is dense (n == N_MAX) a whole pass is one contiguous
// burst (1 instruction for G matrices); otherwise rows are copied one DataCopyPad at a
// time, exactly as step-4 did. No strided bursts, so no reliance on gap-vs-start
...
public:
    ...
    static constexpr int32_t G_MAX = 8;          // matrices per vector pass
...
private:
    ...
    __aicore__ inline void LoadRows(int32_t n, int32_t g, int64_t base, LocalTensor<float> mat) {
        ...
        if constexpr (std::is_same<DT_LOGITS, float>::value) {
            if (n == N_MAX) {   // dense window: the whole pass is one contiguous burst
                DataCopyExtParams all{1, static_cast<uint32_t>(g * WIN * sizeof(float)), 0, 0, 0};
                DataCopyPad(mat, x_gm_[base], all, pp);
            } else {
                for (int32_t w = 0; w < g; ++w) {
    ...
    __aicore__ inline void StoreRows(int32_t n, int32_t g, int64_t base, LocalTensor<float> mat) {
        ...
        if constexpr (std::is_same<DT_LOGITS, float>::value) {
            if (n == N_MAX) {
                DataCopyExtParams all{1, static_cast<uint32_t>(g * WIN * sizeof(float)), 0, 0, 0};
                DataCopyPad(o_gm_[base], mat, all);
            } else {
                for (int32_t w = 0; w < g; ++w) {
                    for (int32_t i = 0; i < n; ++i) {
