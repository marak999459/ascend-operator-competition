...
| ✅ 提交源当前状态 | `op_kernel/mhc_sinkhorn.cpp` = **step-6b 多矩阵打包 `G_MAX=31`（`668c24d7`，327 行）**，证据三层半：真机 fp32 常量 6/6 + 随机 3/3（6 配置含 `1024×8`/`8192×8` ⇒ `g=26`、`g=31` 两条新路都在真机跑过）、真机 fp16 6/6、仿真机 **41 项全 PASS**（`1024` 各 n 逐元素 `mism=0`、确定性 `bitdiff=0`）、**平台待提交**（§11.20）。AGENT.MD 硬约束 5：等用户确认。回滚链：step-6a `0b7af944`（`.bak_vec6a`）→ **step-5 `d50b94d4`（平台 5/5 的那个版本，`.bak_vec5`）** → 标量版 `.bak` `0121995bbae2` → step-1 `bdd8dfa8`（git `686f783` 里） |
| 性能 | 平台 5 个 case 原值 **8.4 / 14.62 / 9.3 / 15.14 / 25.94 µs**（§11.17，step-5 拿的）。真机设备时长（fp32, iters=20）`1024×8`：step-4 116.964 → step-5 37.412 → step-6a 31.052 → **step-6b 28.163（两轮累计 −24.7%，§11.20）**；标量版 3048.97。`2000×8` 66.249 → **51.008**。"303µs 慢 200 倍"那条无形状前缀的历史数已作废（§11.2） |
...
**一句话**：正确性在真机 fp32/fp16 + 仿真机三处对齐（step-6b 三层证据齐），性能累计 `1024×8` −24.7%；
**V 侧的钱已经榨干**（打包到 API 上限、`n` 不改变块数），下一步最便宜的动作不是改代码，
是**提交 step-6b 拿平台 5 个 case 的逐项增减做归因**——它回答"判分形状到底是不是大 batch"，
而这个问题决定还要不要为算子再投一轮工（§11.20 下一步 ①）。
