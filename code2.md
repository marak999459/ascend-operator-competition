...
| 正确性 | ✅ **平台 5/5 已由向量版连续两次拿到**：step-5 `6ab0bc78…`（§11.17）与 step-6b `6ab0cd43…`（§11.21），**两次都是 5 个 case 每条 `precision_ratio: 1`**；更早那次 5/5 是标量版 |
| ✅ 提交源当前状态 | `op_kernel/mhc_sinkhorn.cpp` = **step-6b 多矩阵打包 `G_MAX=31`（`668c24d7`，327 行）**，**§11.9 的四层证据已全部集齐**：真机 fp32 常量 6/6 + 随机 3/3（6 配置含 `1024×8`/`8192×8` ⇒ `g=26`、`g=31` 两条新路都在真机跑过）、真机 fp16 6/6、仿真机 **41 项全 PASS**（`1024` 各 n 逐元素 `mism=0`、确定性 `bitdiff=0`）、**平台 `Pass` 5/5**（§11.21）。回滚链：step-6a `0b7af944`（`.bak_vec6a`，git 里同名 `refs/.../k_vec6_g16.cpp`）→ **step-5 `d50b94d4`（`.bak_vec5` = git `HEAD~1`，三处一致已抽查）** → 标量版 `.bak` `0121995bbae2` → step-1 `bdd8dfa8`（git `686f783` 里） |
| 性能 | ⚠️ **平台 `time` 不可用作 A/B 判据**（§11.21：同一条指令流上噪声 ±15%，本轮信号只有 ~9%）：step-5 → step-6b 读数为 8.40→9.58 / 14.62→14.28 / 9.30→10.70 / 15.14→16.20 / 25.94→25.54。**涨跌以本地真机 `aiv_vec_time` 为准**：`1024×8` 30.113 → 25.494 → **23.264（−8.7%）**、总时长 step-4 116.964 → step-5 37.412 → step-6a 31.052 → **step-6b 28.163（两轮累计 −24.7%）**；`2000×8` 66.249 → **51.008**；标量版 3048.97。"303µs 慢 200 倍"那条无形状前缀的历史数已作废（§11.2） |
| 榜单状态 | ⚠️ **`ranking_submission_mode = latest`**（§11.21）⇒ 在榜的是**最后一次**提交 = **step-6b `6ab0cd43`**，不是最好一次。配额 50 次/天。任何"试验性提交"结束后都要明确收留在榜版本 |
| 病根（当前认知） | 成本模型已标定到能用：`每趟 V 条数 = 184 + g`、`每条 = 固定 12.8ns + 3.63ns/窗口组`，g=1..26 三个实测点偏差 ≤5.5%（§11.20）。⇒ **打包轴结案**：`G_MAX=31` 是 `uint8_t repeatTime` 的硬上限，且固定项只剩单条指令的 12%。⭐ **新发现（§11.20 #3）：V 时长对 `n` 几乎不敏感** —— `1024×4` 的数据是 `1024×8` 的 1/4、V 却是 104%（三个版本九个读数一致）⇒ mask 不减少走的块数，整条 V 路径几何锁死在 8 行窗口，**候选 B（按 n 省数据量）判死**；`n<8` 剩下的 31~38% 堆是逐行 DMA，与 V 侧是同一堵墙 |
| 目标平台配置 | `.AddConfig("ascend910b")` |

**一句话**：step-6b 已把 §11.9 的四层证据集齐（真机 fp32/fp16 + 仿真机 41 项 + 平台 5/5 两次），
本地口径 `1024×8` 两轮累计 **−24.7%**；**V 侧的钱到此榨干**（打包到 API 上限、`n` 不改变块数），
而这一轮**新买到的结论比性能数更重要**：平台的 `time` 读数噪声 ±15% 不能当 A/B 用，且排名取"最后一次提交"
⇒ 下一步要么是花 2~3 次提交做 A/A+A/B/A 把在榜版本定死，要么转到 `normOut`/`sumOut` 契约未核项，**不再抠 V**。
