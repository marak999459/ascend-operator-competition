...
| 病根（当前认知） | 成本模型已标定到能用：`每趟 V 条数 = 184 + g`、`每条 = 固定 12.8ns + 3.63ns/窗口组`，g=1..26 三个实测点偏差 ≤5.5%（§11.20）。⇒ **打包轴结案**：`G_MAX=31` 是 `uint8_t repeatTime` 的硬上限。⭐ **V 时长对 `n` 不敏感**（§11.20 #3）⇒ mask 不减少走的块数，几何锁在 8 行窗口。~~`n<8` 剩下的 31~38% 堆与 V 侧是同一堵墙~~ **§11.22 推翻**：那是逐行 DMA，官方 `CopyIn` 有现成解法，已砍成每方向 n 条步进突发 ⇒ **剩下的唯一大靶子 = V 几何本身**（`1024×6` 的 vec 占 75%、`1024×4` 占 72%），要动就得走 §11.22 ① 的稠密槽布局 |
...
本地三层证据齐、合规 grep 零命中，**没上平台**。比这两个数更值钱的两件事：
...
时它自己已占一块"（⚠️ 更正 `code1.md §14.9` 的"两侧都按字节"，该文件由别路会话在改，未跨界）。
⇒ 现在唯一的系统性大靶子回到 V 几何（占 `n<8` 总时长 72~75%），要不要为它做 step-8 稠密槽布局
（4~6 小时、只收益于 `n∈{4,6}`、判分形状平台不暴露）与要不要把 step-7 投上去占住榜首，**都要用户点头**。
...
`code1.md` 当前由别的路径会话在改，**我没有跨界去动它**，在此登记为待办。
...
② **要不要把 step-7 投上去**（1 次提交、~2 分钟，dry-run 已过只差 `--no-wait`）：三层本地证据已齐；`ranking_submission_mode = latest`
