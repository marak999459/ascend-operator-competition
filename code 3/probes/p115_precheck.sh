#!/bin/bash
# P115 发次链（§3 / §7.5 固定顺序）：探针与闸门都跑完了 → **最后一次 sync（此后不再 build、不再 sed）**
# → 核四枚当场 sha256（本地 vs 远端）+ SoC sed 残留 0 + 禁用词 0 + 命中数 → dry-run 四枚逐个吻合。
# ⛔ 这一发只做到 dry-run；submit 由 p115_submit.sh 单独跑（读数要留在另一段里）。
set -u
...
nssh "cd ~/sfa_real/code && sha256sum $FILES | cut -c1-16,66- ; \
  ...
  echo HITS_P115=\$(grep -c 'P115 split' op_kernel/sparse_flash_attention.cpp) WHOLELEFT=\$(grep -c 'CopyGm2Ub(kb\[done \* D_\],  kGm_\[kOff\],  run \* D_)' op_kernel/sparse_flash_attention.cpp) ; \
  echo RESIDUE=\$(grep -c 'P114\|P112 探针\|P110 rep\|probeRep_' op_kernel/sparse_flash_attention.cpp) ; \
