#!/bin/bash
# P118 本地标定：四臂 = pristine / 暖行×1 / 冷行×1 / 冷行×2（同位置、同条数、同 dst，只差冷暖与组数）。
# 目的：① 惰性自证（三臂输出与 pristine 逐位一致）；② 本地冷暖价差（预期小，本地缓存兜得住 ⇒ 只是量级）。
...
REPS="${REPS:-5}"
# 臂 = "N MODE"（0 pristine 不铺补丁）
ARMS="${ARMS:-0 1warm 1cold 2cold}"
...
( cd "$REPO/code 3/probes" && tar cf - p118_coldcopy.py ) | nssh "cd ~/sfa_real && tar xf -"
echo ">>> pristine sha 本地=$sha_local 远端应同"
nssh "cd ~/sfa_real && sha256sum kernel_pristine.cpp | cut -c1-16" 2>&1 | grep -av Warning | tail -1

for A in $ARMS; do
  NN="${A%%[a-z]*}"; N="${A#$NN}"
  echo "########## ARM $A (N=$NN MODE=${N:-na}) ##########"
  if [ "$A" = "0" ]; then
    cat "$PRISTINE" | nssh "cat > ~/sfa_real/code/op_kernel/sparse_flash_attention.cpp"
  else
    nssh "cd ~/sfa_real && python3 p118_coldcopy.py $NN $N code/op_kernel/sparse_flash_attention.cpp kernel_pristine.cpp" 2>&1 | grep -av Warning | tail -1
  fi
  ...
  nssh "cd ~/sfa_real/code && echo HITS_P118=\$(grep -c 'P118 gather meter' op_kernel/sparse_flash_attention.cpp)" 2>&1 | grep -av Warning
  ALIVE=0
  ...
  # 惰性自证：pristine 臂写一次性 golden_p118/，后三臂与之逐位比（冷大档上也要有证据，不能只在 p6 上）
  ACT=write; [ "$A" = "0" ] || ACT=diff
  nssh "$ENVR; for c in $CASES; do printf '%-11s ' \$c; ./test_sfa_dev cases/\$c.bin 1 $ACT 2e-3 1e-2 golden_p118 2>&1 | grep -aoE '逐位一致 [^ ]+|不\*\*逐位一致\*\*|golden 写入 [^:]+: [A-Z]+' | tr '\n' ' '; echo; done" 2>&1 | grep -av "^Warning"
