#!/bin/bash
# P128 发次链（§7.5 固定顺序）：数值门已跑完（p128_gate.sh）⇒ **最后一次 sync（此后不再 build、不再 sed）**
# → 核四枚当场 sha256（本地 vs 远端）+ SoC sed 残留 0 + 禁用词 0 + 护栏命中数 → dry-run 四枚逐个吻合。
# ⛔ 这一发只做到 dry-run；submit 由 p128_submit.sh 单独跑（读数要留在另一段里）。
set -u
...
echo ">>> 期望 P128=2 BESTUNITS=3 OLDGATE=0 BC0=2 OLDTAB=1 KRES=0 FORBID=0 SOCEDEX=0；"
echo ">>>      host=f405dc065be30ecf，kernel 三枚 = f815bf1e / f2b28a86 / 1046b349（一发都没动）"
echo "⚠️ KRES 的门用 'P1[12][0-9]'：pristine kernel 里合法留着 'P105' 注释（发货字节的一部分，不是探针残留）。"
echo "⚠️ OLDTAB=1 是**对的**：P128 不动候选表（那是 P126 的钉法），只在 L304 的改档门上套护栏。"
