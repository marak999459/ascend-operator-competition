#!/bin/bash
# P124 发次链（§3 / §7.5 固定顺序）：闸门跑完 → **最后一次 sync（此后不再 build、不再 sed）**
# → 核四枚当场 sha256（本地 vs 远端）+ SoC sed 残留 0 + 禁用词 0 + 命中数 → dry-run 四枚逐个吻合。
# ⛔ 这一发只做到 dry-run；submit 由 p124_submit.sh 单独跑（读数要留在另一段里）。
set -u
...
nssh "cd ~/sfa_real/code && sha256sum $FILES | cut -c1-16,66- ; \
  ...
  echo MARK=\$(grep -c 'P124 死端孪生计量器' op_kernel/sparse_flash_attention.cpp) \
  ...
  echo RESIDUE=\$(grep -c 'P123\|p123\|Muls(oc, oc, 1.0f\|P122\|P121\|p121\|P120 flag meter\|SetFlag<HardEvent::MTE2_V>(4)\|P119 PV meter\|skP119\|rP119\|P118 gather meter\|P115 split\|P116 softmax-rep\|P114\|P112 探针\|P110 rep\|probeRep_' op_kernel/sparse_flash_attention.cpp) ; \
...
N="${N:-15}"
echo ">>> 期望 MARK=1 LOOP=1 INNER=1 MULS=12 FLAG=36 BUFF=13 RESIDUE=0 FORBID=0 SOCEDEX=0（N=$N 只进注释与循环常量，行数增量恒 +11）"
