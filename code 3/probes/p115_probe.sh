#!/bin/bash
# P115 发次前置：把 split=2 铺进**本地提交源**（探针要真的发上平台）→ sync → sed → build
# → 命中数门（§7 第 2 条）→ p32_gate 数值闸门（真 golden 双 dtype）。
# ⚠️ 从 pristine（= P105 字节 f815bf1e）重生成，不叠在已有探针上；铺完的本地源就是待发字节。
# 退出 trap 不做还原：探针态是要发出去的东西，还原由 p115_reissue.sh 负责。
set -u
...
echo "########## 1) 本地铺 split=2 ##########"
python3 "$REPO/code 3/probes/p115_split.py" 2 "$K" "$PRIST"
echo "本地 HITS_P115=$(grep -c 'P115 split' "$K")  残留整条拷贝=$(grep -c 'CopyGm2Ub(kb\[done \* D_\],  kGm_\[kOff\],  run \* D_)' "$K")"
[ "$(grep -c 'P115 split' "$K")" = "3" ] || { echo ">>> 命中数 != 3 ⇒ 中止"; exit 1; }
...
echo "########## 3) 远端命中数门 ##########"
nssh "cd ~/sfa_real/code && echo HITS_P115=\$(grep -c 'P115 split' op_kernel/sparse_flash_attention.cpp) \
  WHOLELEFT=\$(grep -c 'CopyGm2Ub(kb\[done \* D_\],  kGm_\[kOff\],  run \* D_)' op_kernel/sparse_flash_attention.cpp) \
  P114=\$(grep -c 'P114\|probeRep_' op_kernel/sparse_flash_attention.cpp) \
