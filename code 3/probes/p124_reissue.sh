#!/bin/bash
# P124 探针读完 ⇒ 回发当前最好的字节（P105，latest 计分）。
# 链：本地还原 → 最后 sync（不再 build）→ 核四枚 sha + SoC 0 + 全部探针标记 0 + 禁用词 0 → dry-run 逐个吻合 → submit
set -u
