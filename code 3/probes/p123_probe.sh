#!/bin/bash
# P123 发次前置：把"链上多串 N 圈"计量器铺进**本地提交源** → sync → sed → build
# → 命中数门（§7 第 2 条）→ p32_gate 数值闸门（真 golden 双 dtype）。
# ⚠️ 从 pristine（= P105 字节 f815bf1e）重生成，不叠在已有探针上；铺完的本地源就是待发字节。
# ⛔ 前置：p123_calib.sh 必须已给出 **Δ∝N**（本地四臂），否则 `x*1.0f` 被折叠 ⇒ 尺子不存在，不许发。
# 退出 trap 不做还原：探针态是要发出去的东西，还原由 p123_reissue.sh 负责。
set -u
N="${1:-15}"
...
EXP_LINES=10   # 4 行注释 + 6 行循环体，与 N 无关

echo "########## 0) pristine 校验 (N=$N) ##########"
sha256sum "$PRIST" "$K" | cut -c1-16,76-
[ "$(sha256sum "$K" | cut -c1-16)" = "$(sha256sum "$PRIST" | cut -c1-16)" ] || \
  { echo ">>> 本地源 != pristine(P105) ⇒ 中止（先跑 p123_reissue.sh 还原，避免叠补丁）"; exit 1; }

echo "########## 1) 本地铺 N=$N ##########"
python3 "$REPO/code 3/probes/p123_chainrep.py" "$N" "$K" "$PRIST"
echo "本地 MARK=$(grep -c 'P123 依赖链计量器' "$K")(应=1) LOOP=$(grep -c 'p123r = 0u' "$K")(应=1) \
INNER=$(grep -c 'Muls(oc, oc, 1.0f, rowC)' "$K")(应=1) MULS=$(grep -c 'Muls(' "$K")(应=12) \
FLAG=$(grep -cE '(Set|Wait)Flag<HardEvent::' "$K")(应=36) 新buffer=$(grep -c 'InitBuffer' "$K")(应=13) \
行差=$(($(wc -l < "$K") - $(wc -l < "$PRIST")))(应=$EXP_LINES)"
[ "$(grep -c 'P123 依赖链计量器' "$K")" = "1" ] || { echo ">>> 标记命中数 != 1 ⇒ 中止"; exit 1; }
[ "$(grep -c 'p123r = 0u' "$K")" = "1" ] || { echo ">>> 外层计量循环 != 1 ⇒ 中止"; exit 1; }
[ "$(grep -c 'Muls(oc, oc, 1.0f, rowC)' "$K")" = "1" ] || { echo ">>> 内层链 != 1 ⇒ 中止"; exit 1; }
...
echo "########## 3) 远端命中数门 ##########"
nssh "cd ~/sfa_real/code && echo MARK=\$(grep -c 'P123 依赖链计量器' op_kernel/sparse_flash_attention.cpp) \
  LOOP=\$(grep -c 'p123r = 0u' op_kernel/sparse_flash_attention.cpp) \
  INNER=\$(grep -c 'Muls(oc, oc, 1.0f, rowC)' op_kernel/sparse_flash_attention.cpp) \
  ...
  OLD=\$(grep -c 'P122\|P121\|p121\|P120 flag meter\|SetFlag<HardEvent::MTE2_V>(4)\|P119 PV meter\|skP119\|P118 gather meter\|P115 split\|P116 softmax-rep\|P114\|P112 探针\|P110 rep\|probeRep_' op_kernel/sparse_flash_attention.cpp) \
