#!/bin/bash
# P115 本地标定：同一份 pristine 三臂 = 每条 gather 拆成 1 / 2 / 4 段（同总字节、调用数 ×N、逐位惰性）。
# 配 P114（字节 ×2、调用 ×2）读"调用成本 vs 字节成本"的分解；平台那一侧由 P115 发次给。
...
( cd "$REPO/code 3/probes" && tar cf - p115_split.py ) | nssh "cd ~/sfa_real && tar xf -"
nssh "cd ~/sfa_real && sha256sum kernel_pristine.cpp | cut -c1-16" 2>&1 | grep -av Warning | tail -1
echo ">>> pristine sha 本地=$sha_local 远端应同"

for N in $NS; do
  echo "########## ARM split=$N ##########"
  nssh "cd ~/sfa_real && python3 p115_split.py $N code/op_kernel/sparse_flash_attention.cpp kernel_pristine.cpp" 2>&1 | grep -av Warning | tail -1
  ...
  # 惰性自证：N=1 臂写 golden_p115/，后两臂与之逐位比（冷大档上也要有证据，不能只在 p6 上）
  ACT=write; [ "$N" = "1" ] || ACT=diff
  nssh "$ENVR; for c in $CASES; do printf '%-11s ' \$c; ./test_sfa_dev cases/\$c.bin 1 $ACT 2e-3 1e-2 golden_p115 2>&1 | grep -aoE '逐位一致 [^ ]+|不\*\*逐位一致\*\*|golden 写入 [^:]+: [A-Z]+' | tr '\n' ' '; echo; done" 2>&1 | grep -av "^Warning"
