#!/bin/bash
# P121 发次链（§3 / §7.5 固定顺序）：闸门跑完 → **最后一次 sync（此后不再 build、不再 sed）**
# → 核四枚当场 sha256（本地 vs 远端）+ SoC sed 残留 0 + 禁用词 0 + 命中数 → dry-run 四枚逐个吻合。
# ⛔ 这一发只做到 dry-run；submit 由 p121_submit.sh 单独跑（读数要留在另一段里）。
set -u
...
nssh "cd ~/sfa_real/code && sha256sum $FILES | cut -c1-16,66- ; \
  ...
  echo MARK=\$(grep -c 'P121 扫描计量器' op_kernel/sparse_flash_attention.cpp) \
  LOOP=\$(grep -c 'p121r = 0u' op_kernel/sparse_flash_attention.cpp) \
  INNER=\$(grep -c 'p121i = p121Lo' op_kernel/sparse_flash_attention.cpp) \
  ...
  echo RESIDUE=\$(grep -c 'P120 flag meter\|SetFlag<HardEvent::MTE2_V>(4)\|P119 PV meter\|skP119\|rP119\|P118 gather meter\|P115 split\|P116 softmax-rep\|P114\|P112 探针\|P110 rep\|probeRep_' op_kernel/sparse_flash_attention.cpp) ; \
...
N="${N:-15}"
echo ">>> 期望 MARK=1 LOOP=1 INNER=1 SINK=1 READS=2 FLAG=36 BUFF=13 RESIDUE=0 FORBID=0 SOCEDEX=0（N=$N 只进注释与循环常量，行数增量恒 +21）"
