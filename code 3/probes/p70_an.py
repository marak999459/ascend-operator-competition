#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P70 判读：dump 的 attention_out 前 16 格 = AIV 从环里读回的 **Gram 行**（B 侧喂 A tile）。
    ...
    for b in range(B):
        for s in range(min(S1, nrow)):
            ...
            for h in range(N1):
                ...
                exp = [sc * G[h][j] for j in range(16)]
                bad, nan = [], []
                for j in range(16):
                    if got[j] != got[j]:
                        nan.append(j)
            ...
            if s == 0:
                ...
                print('  对角（读回）: %s   LSE max 列: %s' %
                      (' '.join('%8.4f' % ou[base + h * 512 + h] for h in range(N1)),
...
if __name__ == '__main__':
    DUMP = sys.argv[3] if len(sys.argv) > 3 else '/tmp/p70/dump'
    for cs in (sys.argv[1:3] or ['p1', 'p2']):
        main(cs, '/tmp/p68/%s.bin' % cs if os.path.exists('/tmp/p68/%s.bin' % cs)
             else '/tmp/p70/%s.bin' % cs, DUMP)
