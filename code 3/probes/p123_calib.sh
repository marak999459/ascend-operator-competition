#!/bin/bash
# P123 本地标定：四臂 = pristine / 链上多串 1 圈 / 3 圈 / 7 圈（同一份 pristine 现生成，只差圈数 N）。
...
# ③ 逐位惰性：八档 out/max/sum 对一次性 golden_p123/ 逐位比 —— 这一条不过就是补丁本身错了。
...
( cd "$REPO/code 3/probes" && tar cf - p123_chainrep.py ) | nssh "cd ~/sfa_real && tar xf -"
...
for N in $ARMS; do
  ...
  nssh "cd ~/sfa_real && python3 p123_chainrep.py $N code/op_kernel/sparse_flash_attention.cpp kernel_pristine.cpp" 2>&1 | grep -av Warning | tail -1
  ...
  WANT=$((N>0 ? 1 : 0))
  nssh "cd ~/sfa_real/code && echo HITS_P123=\$(grep -c 'P123 依赖链计量器' op_kernel/sparse_flash_attention.cpp) \
    LOOP=\$(grep -c 'p123r = 0u; p123r < ' op_kernel/sparse_flash_attention.cpp) \
  ...
  # 惰性自证：pristine 臂写一次性 golden_p123/，后三臂与之逐位比
  ACT=write; [ "$N" = "0" ] || ACT=diff
  nssh "$ENVR; for c in $CASES; do printf '%-11s ' \$c; ./test_sfa_dev cases/\$c.bin 1 $ACT 2e-3 1e-2 golden_p123 2>&1 | grep -aoE '逐位一致 [^ ]+|不\*\*逐位一致\*\*|golden 写入 [^:]+: [A-Z]+' | tr '\n' ' '; echo; done" 2>&1 | grep -av "^Warning"
