...
//         P10 起头块也是一维单元；P11v2 起，在"单元数 ×ks 仍不超过核数"时把 sparse 列表
//         也切成 ks 半分给 ks 个核（tiling 的 kv_shard；P18 起按**下标模 ks** 交错分，
//         不按连续区间 —— 见 ProcessToken）。P105 把 ks 从固定 2 推广到任意深度。
//         分片是唯一需要核间通信的一维：分片 0 照常把半行结果写进【本算子自己的输出张量】，
//         然后 ks-1 轮「SyncAll → 分片 r 读回累加器、与自己的部分和合并、再发布」串成链。
//         每一轮的输入/输出格式与分片 0 的 WriteOut 逐字节同构（归一化输出 + (m,l)），
//         所以加深 ks 不引入新语义，只是多几轮 fp16 往返和几笔 barrier 延迟。
...
//         ⚠️ SyncAll 口径（§15.16/§15.17/§15.32）：所有块走到、次数一致才是安全的
//         （arch22 + AIV-only）；上道题死锁的根因是"各核调用次数不齐"。P105 的 ks-1 轮
//         循环满足这一条：SyncAll 在 r 循环的循环体首、不受任何核私有条件分支约束。
//         一轮"跨核折叠"实测 1.7~11.8 µs（消融臂多走一遍阶段 B，见 LOG#15.86）⇒ 延迟主导、
//         不随 payload 缩放；块数 1/16/40 实测都能过 barrier ⇒ 与 P14 收缩块数不冲突。
...
public:
                                ...
                                const SparseFlashAttentionTilingData &tiling_data, GM_ADDR trace)
    {
        ...
        // ---- P11v2/P105：sparse 列表（KV 轴）切成 ks_ 半分给 ks_ 个核 ----
        // host 只在"切完仍是【一核一单元】"时才下发 kv_shard>=2；这里再核一遍，因为归并
        // 要用的分片部分和是留在 UB 里的（同核再领一个单元就被覆盖），而 lseOn_=false
        ...
        const uint32_t ksh = tiling_data.kv_shard;
        if (ksh >= 2u && ksh <= 64u && lseOn_ && total0 > 0u && coreNum == ksh * total0) {
            ks_ = ksh;
        }
        if ASCEND_IS_AIC {
    ...
    __aicore__ inline void Process()
    {
        ...
        if (ks_ < 2u) { return; }
        // ---- 阶段 B：ks_-1 轮链式折叠，每轮一道 barrier ----
        // 第 r 轮：GM 里的累加器（= 分片 0..r-1 已合并、已归一发布的"整段部分和"）由分片 r-1
        // 写下，分片 r 读回它、与自己 UB 里的未归一部分和合并、再按同一格式发布。
        // MergeToken 的输入/输出与分片 0 的 WriteOut 同构 ⇒ 深度只是多走几轮，没有新语义。
        // ⚠️ barrier 必须在 r 循环体首、无条件：所有块都恰好走 ks_-1 次，不存在
        //    §15.16 那类"各核调用次数不齐"的死锁形态（ks_ 由各块同一份 tiling 推出）。
        for (uint32_t r = 1u; r < ks_; ++r) {
            SyncAll();
            for (uint32_t u = unitBegin_; u < unitEnd_; u += unitStep_) {
                const uint32_t unit  = u / ks_;
                const uint32_t shard = u - unit * ks_;
...
private:
    ...
    //   shard 0 —— 照常 WriteOut，它的 attention_out / LSE 既是最终结果（其余分片空时）
    //              也是分片 1 折叠时要读的"已发布半行"（所以这里不需要任何新的发布代码）；
    //   shard>0 —— 不写回，部分和 O(未归一) + m + l 留在 oBuf_/mlBuf_ 里，轮到自己的那一轮
    //              由 MergeToken 收口（每核一个单元 ⇒ 中间不会被覆盖，见 Init 的门）。
    __aicore__ inline void ProcessToken(uint32_t b, uint32_t s, uint32_t headBlk, uint32_t shard)
    {
        ...
        // ⚠️ 只有分片 0 发布：它写下的就是第 1 轮 MergeToken 要读回的"已发布半行"。其余
        //    分片的部分和（未归一的 O、m、l）留在 UB 里，轮到自己的那一轮才收口。
        if (shard == 0u) {
    ...
    /**
     * P11v2/P105: 第 r 轮折叠 —— 读回 GM 里的累加器（分片 0..r-1 的合并结果），与本分片
     * UB 里的部分和合并，覆盖写 attention_out 与 LSE。对 r=1 而言"累加器"就是分片 0 的
     * WriteOut 输出；对 r>1 就是上一轮 MergeToken 的输出（同一格式，故可串成链）。
     *
     * 在线 softmax 的合并式（Ô0 = 累加器里已归一的输出，O1/m1/l1 = 本分片 UB 的未归一部分和）：
     *     m  = max(m0, m1)
     *     w0 = l0 * exp(m0 - m)         u1 = exp(m1 - m)         L = w0 + l1 * u1
     *     O  = (Ô0 * w0 + O1 * u1) / L
     * 与"整行一次算完"的差别只有求和顺序，外加累加器每轮多走一次 DT_QUERY 往返（fp16 一个
     * ULP 量级，远小于判分口径 atol=2e-3/rtol=1e-2）。⚠️ ks 深度是真成本：链上每一次
     * 往返都重新量化同一个累加器，最坏情形 ≈ (ks-1)·2^-11 相对误差（ks=10 ⇒ 4.4e-3，仍在
     * rtol=1e-2 内，但**必须**实测超差数，见 code3.md §1 精度闸门）。
     * ⇒ 切分的用例不再与参考实现【逐位】相同，这正是 host 把 kv_shard 的门卡得很紧
     *    （只给"一核一单元"的小形状）的原因。
     *
     * ⚠️ 前一分片的 (m0,l0) 只能批量读：GM 标量 GetValue 不是跨核通道（§15.32 实测标量
     ...
     *      l1 <= 0：累加器写的就是最终值 ⇒ 按原值透传（同址同字节，重写无害）；
     *      l0 <= 0：最终值取本分片 ⇒ m=m1、L=l1、O=O1/l1（累加器那项权重为 0，不读它）。
     */
    __aicore__ inline void MergeToken(uint32_t b, uint32_t s, uint32_t headBlk)
    {
        ...
        LocalTensor<DT_QUERY> kb0 = kBuf_.Get<DT_QUERY>();  // 累加器输出的落地区（chunk 循环后空闲）
        LocalTensor<DT_QUERY> st  = kfBuf_.Get<DT_QUERY>();
