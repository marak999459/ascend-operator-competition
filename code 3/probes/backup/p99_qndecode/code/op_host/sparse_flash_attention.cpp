...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    // ---- P99 探针：把"平台六点各自落在哪道门外"编码进并行度，一枪读六点 ----------------
    // 已知（P98-F，9/23 夜）：六点全是 **fp16**（只给 fp32 夹 blockDim 的判别器六点全在
    // ±7 % 噪声带里 ⇒ fp32 分支一次都没进）。那么 A′ 形态门里能六点一律关死的判据只剩：
    //   ① `qD!=512 || dr!=64`（题面钉死，但我们整套本地语料都建在这条上，必须量）；
    //   ② `CubeBlock(qN)`：qN==1 时 `qN & ~1U == 0` ⇒ `nb<2` 直接拒 —— MLA-absorb 这个
    //      题名本身就把头数往 1 上逼，而 §1.3 那张单价表**从来没有 qN=1/2 那一列**；
    //   ③ qN ∈ {2,4,8}：门该开（本地这三档实测 1.67~2.18×），可平台上三点零响应；
    //   ④ qN ≥ 16：A′ 就是为它做的，门该开，零响应 ⇒ 只剩构建层。
    // lever 与 P98-F 同源且已证数值惰性：kernel 是 grid-stride（`unitStep_=GetBlockNum()`）
    // ⇒ `blockDim /= div` **一个比特都不改**，只把时间乘上去。四档 div = 8/4/2/1 两两相差
    // 2×，远大于平台单点 ±7 % 噪声 ⇒ 每点读到的倍率就是它的档位。
    // ⚠️ 优先级编码：①最外层（形状若不对，后面全无意义），所以①拿最响的一档。
    uint32_t bdDiv = 1U;
    if (dtype_query == ge::DT_FLOAT16) {
        if (Q_D != HQ_DIM || Dr != ROPE_DIM) { bdDiv = 8U; }
        else if (Q_N <= 1U)                 { bdDiv = 4U; }
        else if (Q_N <= 8U)                 { bdDiv = 2U; }
    }
    ...
    //         kv_shard 当 1 用（多启动的那一半块直接空转），结果与不切分逐位相同。
    {
        ...
        blockDim /= bdDiv;                        // P99：grid-stride ⇒ 只改并行度，不改数值
