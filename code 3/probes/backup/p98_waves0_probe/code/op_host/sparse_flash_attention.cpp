...
// P98：本地那一张胜负表是在 `每行 2048 token / 32 片` 的形状族上量出来的，而门槛里的
//      成本项（每片锁步固定税）**不随每单元的片数走** ⇒ 片数更多的形状上门槛要往左移。
//      这一档把并行度门整条让给形态门判，用六点各自的响应反查平台形状落在哪一侧。
constexpr uint64_t CUBE_MIN_WAVES_PCT = 0ULL;    // 0 ⇒ 只留形态门
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    uint32_t cubeOn = 0U;
    {
        ...
        // ⚠️ `CubeBlock(1) == 0`（1 头凑不满 m 轴的 16 行分形，形态门紧接着就拒），
        //    但这里先要它做除数 ⇒ qN=1 的输入会在 tiling 里整数除零。空块按 0 个单元算。
        const uint64_t cblkN = (cblk == 0U) ?
            0ULL : (static_cast<uint64_t>(Q_N) + cblk - 1ULL) / cblk;
        const uint64_t cubeUnits = static_cast<uint64_t>(B) * static_cast<uint64_t>(Q_S) * cblkN;
