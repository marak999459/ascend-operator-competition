...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    //
    // P11v2/P105：再往下的话，"单元"这一档的并行度在 rows×Q_N 用完后就没有了，于是把
    //      **sparse 列表**也切成 ks 半分给 ks 个核（kernel 的 kv_shard；P105 起 ks 不再只有
    //      1/2）。归并走本算子自己的输出张量 + 每轮一道 SyncAll，不需要 workspace
    //      （§15.32 实测：跨核只有批量 DataCopy 这条通道是全通的，标量 SetValue/GetValue 不行）。
    //      开启条件那三条（一波内 / LSE 是 8 的整数倍 / 每片装得下一个 chunk）写在
    //      KsAllowed 里，推导见 §15.33(a)(b)。
    ...
    //         kv_shard 当 1 用（多启动的那些块直接空转），结果与不切分逐位相同。
    {
