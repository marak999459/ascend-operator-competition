...
public:
                                ...
                                const SparseFlashAttentionTilingData &tiling_data, GM_ADDR trace)
    {
        ...
        // 一次"整批装载/写回"能暂存的行数（每行 D_ 个元素）。上限取 SFA_SC_GRP：
        // P5b 的写回 scratch 借用 kfBuf_（容量 = SFA_SC_GRP 行 fp32），装载 scratch
        // 借用 kBuf_/krBuf_（容量 = n_blk 行），三者取最小才不会越界，且 host 的 UB
        // 预算式不需要为 P5b 新增任何一项。
        stageMax_ = (nb_ < nBlk_) ? nb_ : nBlk_;
        if (stageMax_ > SFA_SC_GRP) { stageMax_ = SFA_SC_GRP; }
        ...
        pipe_.InitBuffer(vBuf_,   sfa::UbAlignBuf(nBlk_ * D_  * sizeof(DT_QUERY)));
        ...
        // ---- 向量化 scratch ----
        // kfBuf_/krfBuf_：一组 scGrp_ 个 token 的 K/K-rope fp32 展开（每组只 Cast 一次，
        //   供全部头复用）；P2 之后 PV 只借用 kfBuf_ 的一行放 V 的 fp32 展开；
        //   P5b 之后 kfBuf_ 还在 chunk 循环之外兼作 attention_out 的写回暂存区。
        // pfBuf_/prfBuf_：点积的部分积，就地折叠后归约。P13 起 nb_==1 时**不再分配**
        //   （部分积就地写在 kfBuf_/krfBuf_ 上），省下的那份变成 2 倍组宽。
        // rdBuf_：三个宽度为 redW_ 的归约工作区，P3a 用 [0]=content 行和、[redW_]=rope 行和，
        //   P2 复用同两块（[0]=Σp、[redW_]=分块临时）并另用 [2*redW_]=alpha 向量。
        //   ⚠️ 宽度是 redW_ = max(nb_, scGrp_)，不是 nb_：score 的一条 WholeReduceSum 一次出
        //   **g 行**行和（g 最大 = scGrp_），P2 的归约一次出 **nbCur 行** ⇒ 两边都得放得下。
        //   只按 nb_ 定宽时，SFA_SC_GRP > nb_ 会压到下一切片（§15.13：GRP=16 时 r5/big1 全错）。
        // ---- P13：单头单位走"就地广播乘 + 双倍宽组"（口径见 tiling.h 的 SFA_SC_GRP_W）----
        // kfBuf_/krfBuf_ 扩到 scGrp_ 行，pfBuf_/prfBuf_ 在 nb_==1 时**不分配**：
        // 一份 UB 预算换 2 倍组宽，fold/mul/reduce 这些"每组一次"的调用条数直接减半。
        scGrp_ = (nb_ == 1u) ? SFA_SC_GRP_W : SFA_SC_GRP;
        ...
        if (nb_ != 1u) {
            pipe_.InitBuffer(pfBuf_,  sfa::UbAlignBuf(scGrp_ * D_ * sizeof(float)));
            pipe_.InitBuffer(prfBuf_, sfa::UbAlignBuf(scGrp_ * Dr_ * sizeof(float)));
        }
        redW_ = (nb_ > scGrp_) ? nb_ : scGrp_;   // 归约工作区宽度，见下面 rdBuf_ 的注释
...
private:
    ...
    __aicore__ inline void ProcessToken(uint32_t b, uint32_t s, uint32_t headBlk, uint32_t shard)
    {
        ...
        while (true) {
            ...
            if (pend > 0u) {
                FlushChunk(q, o, kb, vb, kr, sc, ml, nbCur, pend);
                pend = 0u;
            }
            ...
            for (uint32_t j = 0u; j < nRun; ++j) {
                ...
                CopyGm2Ub(vb[done * D_],  vGm_[kOff],  run * D_);
                CopyGm2Ub(kr[done * Dr_], krGm_[rOff], run * Dr_);
                done += run;
            }
            pend = cnt;
        }
        if (pend > 0u) { FlushChunk(q, o, kb, vb, kr, sc, ml, nbCur, pend); }
     ...
     *   1) 本组 SFA_SC_GRP 个 token 的 K / K-rope 各整行 Cast 成 fp32 —— 只做一次，
     *      供该组内全部头复用；fp16 -> fp32 是精确扩展，不引入误差。
                                         ...
                                         uint32_t nbCur, uint32_t m)
    {
        ...
        // P13：nb_==1 ⇒ kf/krf 在一组里只被读一次（头循环只有 1 轮），部分积就地下在
        //      kf/krf 上，pfBuf_/prfBuf_ 此时根本没分配。就地写下每个 lane 的两个源操作数
        //      地址相同、无跨 lane 依赖 ⇒ 与写到别的缓冲**逐位等价**。
        const bool inplace = (nb_ == 1u);
        LocalTensor<float> pf = inplace ? kf : pfBuf_.Get<float>();
        LocalTensor<float> prf = inplace ? krf : prfBuf_.Get<float>();
        ...
        for (uint32_t g0 = 0; g0 < m; g0 += scGrp_) {
            ...
            // P12：本组 g 行在 kb/kr 与在 kf/krf 里都是**连续**排布的 ⇒ g 条加宽可以并成
            //      1 条 count=g*rowC 的加宽。写入地址、取值顺序逐位不变，省的是每条调用
            //      固定的 ≈29 cycle（§15.11），与 rep 数无关。
                                      ...
                                      LocalTensor<float> &ml, uint32_t nbCur, uint32_t m)
    {
        ...
        //   MTE2 -> V：本 chunk 的 kb/vb/kr 是刚由 DataCopy 搬进来的，V 读之前
        //     必须等 MTE2 队列走到这个标记点。
    ...
    TPipe pipe_;
    TBuf<TPosition::VECCALC> qBuf_, oBuf_, kBuf_, vBuf_, krBuf_, sBuf_, pBuf_, mlBuf_, lseBuf_;
    TBuf<TPosition::VECCALC> kfBuf_, pfBuf_, krfBuf_, prfBuf_, rdBuf_;
    ...
    bool scBatch_ = false;
    uint32_t redW_ = 1;      // rdBuf_ 每条切片的宽度 = max(nb_, scGrp_)，见 Init
    uint32_t scGrp_ = SFA_SC_GRP;   // 一组多少个 token（nb_==1 时翻倍，见 Init 的 P13 段）
    float scale_ = 1.0f;
    uint32_t mode_ = 3;
    bool lseOn_ = false;
