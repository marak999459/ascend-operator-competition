...
public:
                                ...
                                const SparseFlashAttentionTilingData &tiling_data, GM_ADDR trace)
    {
        ...
        // P81：cube 形态下 AIV 既不搬 K/K-rope（K 由 AIC 自己 ND2NZ 进 L1），也不跑
        //   ComputeScores ⇒ krBuf_/kfBuf_/krfBuf_ 三块【整段不分配】，把 UB 让给 V 与 s/p，
        //   好让 n_blk 抬到 SFA_STAGE_MAX_CUBE。host 的 CalcUbNeed(cube=true) 同一口径。
        ...
        //   UB 块 = 8 个头）。
        scGrp_ = nBlk_;
        // P81：kf/krf 只有 ComputeScores 用 ⇒ cube 形态下同样【不分配】（口径见上面 krBuf_）。
        const uint32_t kfBytes = cubeOn_ ? 0u : sfa::UbAlignBuf(scGrp_ * D_ * sizeof(float));
        const uint32_t krfBytes = cubeOn_ ? 0u : sfa::UbAlignBuf(scGrp_ * Dr_ * sizeof(float));
        if (kfBytes != 0u) { pipe_.InitBuffer(kfBuf_, kfBytes); }
