...
private:
     ...
     *
     * 布局：AIC 用 Fixpipe 写出 [16 行(头) × nBlk_ 列(token)] 的 fp32 ND，行距 = nBlk_，
     * 所以本块 nbCur 行在环里是【连续】的一段 ⇒ 一次 DataCopy 整批读回（P96）。
     *   改动前是"行距 = 本片 nTile"+"每头一条 DataCopy"两件事绑在一起：末片
     *   （cnt<n_blk ⇒ nTile<n_blk）行距与 sBuf_ 的 nBlk_ 不一致，只能逐头搬，于是每 chunk
     *   付 nbCur×≈50 ns 的调用税（§15.70(d) 的单价口径，16 头块 = 0.4 µs/chunk）。
     *   P96 把 AIC 那侧的行距钉成 nBlk_（尾列写垃圾、没人读）⇒ 两侧行距恒等 ⇒ 合成一条。
     *
     * 对齐：dst 偏移 0 个 float（sc 是 sBuf_ 首地址，VECCALC 32B 对齐），长度
     *       nbCur*nBlk_*4 B（nBlk_ 是 16 的倍数 ⇒ 32B 的整数倍）；
     *       src 偏移 (ringBase>>1 + sub_*nb_*nBlk_)*4 B，ringBase = s1Base + 块首*D_，
     *       两项都是 D_=512 的倍数 ⇒ 恒 32B 对齐。
     * 尾列：AIC 按 nBlk_ 列整块写，多出来的列是陈旧值/AIV 从不读（SoftmaxPv 只看前 m 列）。
     */
                                         ...
                                         uint32_t ringBase)
    {
        ...
        // ⇒ 起始行号 = sub_*nb_（P91-A′：块宽可以小于 N1，所以不能再按整份算）。
        DataCopy(sc, ringOutGm_[base + static_cast<uint64_t>(sub_ * nb_) * nBlk_], dcp);
    ...
    __aicore__ inline bool CubeOneChunk(CubeCtx &ctx)
    {
        ...
        // P96：行距锁死成 nBlk_（原来是本片的 nTile）。Mmad 仍然只算 nTile 列，多出来的是
        // L0C 里的陈旧列，而 AIV 的读回按"整块一次 DataCopy"走 ⇒ 尾列必须是【本单元自己的】
        // 区间（环形缓冲借本块输出行，host 的 CubeRingNeed 一直是按最大档 nBlk_ 算的，
        // 所以这次改动的写入区间并没有越到邻块头上）。
        ctx.fx.nSize = static_cast<uint16_t>(nBlk_);
