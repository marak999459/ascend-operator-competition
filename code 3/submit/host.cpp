...
static inline uint64_t CalcUbNeed(uint32_t nb, uint32_t nBlk)
{
    // 与 kernel Init 里的算式逐项对应
    return static_cast<uint64_t>(nb) * QD_ROPE * 4ULL          // qBuf_ : Q + Qrope (fp32)
         + static_cast<uint64_t>(nb) * HQ_DIM * 4ULL           // oBuf_ : 输出累加器 (fp32)
         + static_cast<uint64_t>(nBlk) * HQ_DIM * 2ULL * 2ULL  // kBuf_ + vBuf_ (fp16)
         + static_cast<uint64_t>(nBlk) * ROPE_DIM * 2ULL       // krBuf_ (fp16)
...
static bool CalcBlocking(uint32_t sparseBlockSize, uint64_t ubSafe,
                         uint32_t &nbOut, uint32_t &nBlkOut)
{
    ...
    for (size_t i = 0; i < sizeof(NB_CAND) / sizeof(NB_CAND[0]); ++i) {
        for (size_t j = 0; j < sizeof(NBLK_CAND) / sizeof(NBLK_CAND[0]); ++j) {
            ...
            if (CalcUbNeed(nb, nBlk) > ubSafe) { continue; }
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    const float   scale   = (p_scale != nullptr) ? (*p_scale) : (1.0f / 22.627416997969522f);
    int64_t sbs           = (p_sbs  != nullptr) ? (*p_sbs)  : 1;
    const int64_t mode    = (p_mode != nullptr) ? (*p_mode) : 3;
    const int64_t amode   = (p_amode != nullptr) ? (*p_amode) : 2;

    // sparseBlockSize: 安全映射到 [1,128] 内最近的 2 的幂，不拒绝
    if (sbs < 1) { sbs = 1; }
    if (sbs > 128) { sbs = 128; }
    {
        uint32_t p2 = 1;
        while ((p2 << 1) <= static_cast<uint32_t>(sbs)) { p2 <<= 1; }
        sbs = p2;
    }
    ...
    if (!CalcBlocking(static_cast<uint32_t>(sbs), ubSafe, nb, nBlk)) {
