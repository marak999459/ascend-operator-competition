...
public:
                                ...
                                const SparseFlashAttentionTilingData &tiling_data, GM_ADDR trace)
    {
        ...
        pipe_.InitBuffer(qBuf_,  nb_   * sfa::QD_ROPE * sizeof(float));
        pipe_.InitBuffer(oBuf_,  nb_   * sfa::HQ_DIM  * sizeof(float));
        pipe_.InitBuffer(kBuf_,  nBlk_ * sfa::HQ_DIM  * sizeof(DT_QUERY));
        pipe_.InitBuffer(vBuf_,  nBlk_ * sfa::HQ_DIM  * sizeof(DT_QUERY));
        pipe_.InitBuffer(krBuf_, nBlk_ * sfa::ROPE_DIM * sizeof(DT_QUERY));
...
private:
    ...
    __aicore__ inline void ProcessToken(uint32_t b, uint32_t s)
    {
        ...
        const uint32_t s1Base   = static_cast<uint32_t>(((static_cast<uint64_t>(b) * S1_ + s) * N1_) * sfa::HQ_DIM);
        const uint32_t ropeBase = static_cast<uint32_t>(((static_cast<uint64_t>(b) * S1_ + s) * N1_) * sfa::ROPE_DIM);
        ...
        if (s >= actQ) {
            for (uint32_t n = 0; n < N1_; ++n) {
                const uint32_t outOff = s1Base + n * sfa::HQ_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    outGm_.SetValue(outOff + d, static_cast<DT_QUERY>(0.0f));
                }
                if (lseOn_) {
                    const uint32_t lseOff = (static_cast<uint64_t>(b) * S1_ + s) * N1_ + n;
                    maxGm_.SetValue(lseOff, sfa::SOFTMAX_MIN_NUM);
                    sumGm_.SetValue(lseOff, 0.0f);
                }
            }
            return;
        }
        ...
        for (uint32_t hb = 0; hb < nHeadBlk_; ++hb) {
            ...
            for (uint32_t i = 0; i < nbCur; ++i) {
                const uint32_t qOff = s1Base + (n0 + i) * sfa::HQ_DIM;
                const uint32_t rOff = ropeBase + (n0 + i) * sfa::ROPE_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    float qv = isBf16_ ? sfa::Bf16ToFloat(qRawGm_.GetValue(qOff + d)) : static_cast<float>(qGm_.GetValue(qOff + d));
                    q.SetValue(i * sfa::QD_ROPE + d, qv);
                }
                for (int32_t d = 0; d < sfa::ROPE_DIM; ++d) {
                    float rv = isBf16_ ? sfa::Bf16ToFloat(qrRawGm_.GetValue(rOff + d)) : static_cast<float>(qrGm_.GetValue(rOff + d));
                    q.SetValue(i * sfa::QD_ROPE + sfa::HQ_DIM + d, rv);
                }


            }
            for (uint32_t i = 0; i < nbCur; ++i) {
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) { o.SetValue(i * sfa::HQ_DIM + d, 0.0f); }
            ...
            while (true) {
                ...
                if (filled == nBlk_) {
                    int64_t kBaseGM = (static_cast<int64_t>(b) * S2_ + (curBegin - filled)) * sfa::HQ_DIM;
                    FlushChunk(q, o, kb, vb, kr, sc, ml, nbCur, filled, kBaseGM);
                }
                const int64_t kOff = (static_cast<int64_t>(b) * S2_ + curBegin) * sfa::HQ_DIM;
                const int64_t rOff = (static_cast<int64_t>(b) * S2_ + curBegin) * sfa::ROPE_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    kb.SetValue(filled * sfa::HQ_DIM + d, kGm_.GetValue(kOff + d));
                    vb.SetValue(filled * sfa::HQ_DIM + d, vGm_.GetValue(kOff + d));
                }
                for (int32_t d = 0; d < sfa::ROPE_DIM; ++d) {
                    float rv = isBf16_ ? sfa::Bf16ToFloat(krRawGm_.GetValue(rOff + d)) : static_cast<float>(krGm_.GetValue(rOff + d));
                    kr.SetValue(filled * sfa::ROPE_DIM + d, rv);
                }
                ++filled;
                ++curBegin;
                if (curBegin >= curEnd) { hasBlock = false; }
            }
            if (filled > 0) { int64_t kBaseEnd = (static_cast<int64_t>(b) * S2_ + (curBegin - filled)) * sfa::HQ_DIM; FlushChunk(q, o, kb, vb, kr, sc, ml, nbCur, filled, kBaseEnd); }

            // 归一化并写回
            for (uint32_t i = 0; i < nbCur; ++i) {
                const float l = ml.GetValue(nb_ + i);
                const uint32_t outOff = s1Base + (n0 + i) * sfa::HQ_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    const float val = (l > 0.0f) ? (o.GetValue(i * sfa::HQ_DIM + d) / l) : 0.0f;
                    o.SetValue(i * sfa::HQ_DIM + d, val);
                }
                if (lseOn_) {
                    ...
                    maxGm_.SetValue(lseOff, (l > 0.0f) ? ml.GetValue(i) : sfa::SOFTMAX_MIN_NUM);
                    sumGm_.SetValue(lseOff, (l > 0.0f) ? l : 0.0f);
                }
            }
            for (uint32_t i = 0; i < nbCur; ++i) {
                const uint32_t outOff = s1Base + (n0 + i) * sfa::HQ_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    float oval = o.GetValue(i * sfa::HQ_DIM + d); if (isBf16_) { outRawGm_.SetValue(outOff + d, sfa::FloatToBf16(oval)); } else { outGm_.SetValue(outOff + d, static_cast<DT_QUERY>(oval)); }
                }
            }
        }
    }
                                      ...
                                      LocalTensor<float> &ml, uint32_t nbCur, uint32_t m, int64_t kBaseGM)
    {
        ...
        for (uint32_t i = 0; i < nbCur; ++i) {
            const uint32_t qo = i * sfa::QD_ROPE;
            for (uint32_t j = 0; j < m; ++j) {
                float acc = 0.0f;
                const uint32_t ko = j * sfa::HQ_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    float kv = isBf16_ ? sfa::Bf16ToFloat(kRawGm_.GetValue(kBaseGM + j * sfa::HQ_DIM + d)) : static_cast<float>(kb.GetValue(ko + d));
                    acc += q.GetValue(qo + d) * kv;
                }
                const uint32_t ro = j * sfa::ROPE_DIM;
                for (int32_t d = 0; d < sfa::ROPE_DIM; ++d) {
                    float krv = isBf16_ ? sfa::Bf16ToFloat(krRawGm_.GetValue(kBaseGM + j * sfa::ROPE_DIM + d)) : static_cast<float>(kr.GetValue(ro + d));
                    acc += q.GetValue(qo + sfa::HQ_DIM + d) * krv;
                }
        ...
        for (uint32_t i = 0; i < nbCur; ++i) {
            ...
            const uint32_t oo = i * sfa::HQ_DIM;
            for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
            ...
            for (uint32_t j = 0; j < m; ++j) {
                ...
                lNew += e;
                const uint32_t vo = j * sfa::HQ_DIM;
                for (int32_t d = 0; d < sfa::HQ_DIM; ++d) {
                    float vv = isBf16_ ? sfa::Bf16ToFloat(vRawGm_.GetValue(kBaseGM + j * sfa::HQ_DIM + d)) : static_cast<float>(vb.GetValue(vo + d));
    ...
    TPipe pipe_;
    TBuf<TPosition::VECCALC> qBuf_, oBuf_, kBuf_, vBuf_, krBuf_, sBuf_, pBuf_, mlBuf_, blkBuf_;
