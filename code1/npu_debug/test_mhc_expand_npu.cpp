...
// 返回 0=PASS 1=FAIL 2=ERROR（接口/内存失败，不计入 PASS 数）3=PROF（只做重复计时，不对拍）
// reps>1 时走性能测量路径：先正常跑 1 次，再把同一 executor 连发 reps-1 次并整批计时
static int run_case(const char* name, uint32_t S, uint32_t D, uint32_t m,
                    bool bwd, int dt, int fill_mode, int reps = 0) {
    ...
    do {
        ...
        if (reps > 1) {
            ...
            for (int r = 1; r < reps; r++) {
                if (g_run(dws, ws_size, ex, g_stream) != ACLNN_SUCCESS) { ok = false; break; }
                if (aclrtSynchronizeStream(g_stream) != ACL_SUCCESS) { printf("    [%s] sync failed\n", name); ok = false; break; }
            }
...
#define RUNP(nm, bwd, dt, S_, D_, m_, reps_) do {                                    \
        int r_ = run_case(nm, S_, D_, m_, bwd, dt, 0, reps_);                         \
        done++; if (r_ == 2) err++; else if (r_) fail++;                             \
    } while (0)

    if (prof) {
        // 性能基线专用：fp16 large 前后向各连发 MHC_REPS 次（反向那组是 barrier 的对照组，
        // barrier 只动前向，反向逐条不变 ⇒ 可用来判"环境抖动"还是"改动生效"）
        int reps = getenv("MHC_REPS") ? atoi(getenv("MHC_REPS")) : 21;
        if (reps < 2) reps = 2;
        RUNP("fwd-fp16-large",  false, DT_FP16, 8192, 7168, 8, reps);
        RUNP("bwd-fp16-large",  true,  DT_FP16, 8192, 7168, 8, reps);
    }
    // ===== prof2：一次只跑一条用例（MHC_PCASE 选），让 msprof 的 csv 里只有一组形状 =====
    // 单进程混多形状不行：prof_sum.js 只按 Input Shapes 的维度数分 fwd/bwd，会把小档和
    // 大档的逐任务时间平均成一列。所以每条用例独立 msprof 会话，脚本见 prof_matrix.sh。
    bool prof2 = !strcmp(mode, "prof2");
    if (prof2) {
        struct PC { const char* nm; bool bwd; int dt; uint32_t S, D, m; int reps; };
        static const PC pcs[] = {
            {"fwd-fp16-small",  false, DT_FP16,   64,  256, 2, 401},
            {"bwd-fp16-small",  true,  DT_FP16,   64,  256, 2, 401},
            {"fwd-bf16-small",  false, DT_BF16,   64,  256, 2, 401},
            {"bwd-bf16-small",  true,  DT_BF16,   64,  256, 2, 401},
            {"fwd-fp16-medium", false, DT_FP16, 1024, 4096, 4, 101},
            {"bwd-fp16-medium", true,  DT_FP16, 1024, 4096, 4, 101},
            {"fwd-fp16-large",  false, DT_FP16, 8192, 7168, 8, 21},
            {"bwd-fp16-large",  true,  DT_FP16, 8192, 7168, 8, 21},
        };
        const int n = sizeof(pcs) / sizeof(pcs[0]);
        int sel = getenv("MHC_PCASE") ? atoi(getenv("MHC_PCASE")) : -1;
        if (sel < 0 || sel >= n) { printf("MHC_PCASE must be 0..%d\n", n - 1); return 2; }
        const PC& c = pcs[sel];
        int reps = getenv("MHC_REPS") ? atoi(getenv("MHC_REPS")) : c.reps;
        if (reps < 2) reps = 2;
