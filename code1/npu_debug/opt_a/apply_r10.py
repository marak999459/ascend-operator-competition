...
CONF = {"b1": (1, 2), "b2": (2, 4)}
if MODE not in CONF:
    print("usage: apply_r10.py b1|b2"); sys.exit(1)
BATCH, RING = CONF[MODE]
...
"""
N5 = """    // [R10] 前向批处理驱动：每次攒 FWD_BATCH 个"块"，一起灌 -> 一道 barrier -> 一起吐。
    // 环深 FWD_RING = 2*FWD_BATCH ⇒ 本批写入的格必然不是上一批 MTE3 正在读的格，
    // 于是"上一批的吐出"和"本批的灌入"可以并行，而序的强度与逐块 barrier 完全一样。
    __aicore__ inline void ProcessForward() {
        uint32_t t = task_begin_, j = 0, r = 0;
        while (t < task_end_) {
            uint32_t ii[FWD_BATCH], jj[FWD_BATCH], hh[FWD_BATCH], kk[FWD_BATCH];
            uint32_t nb = 0;
            while (nb < FWD_BATCH) {
            ...
            if (nb == 0) break;
            for (uint32_t u = 0; u < nb; ++u) FwdIn(r + u, ii[u], jj[u], hh[u]);
            PipeBarrier<PIPE_ALL>();
            for (uint32_t u = 0; u < nb; ++u) FwdOut(r + u, ii[u], jj[u], hh[u], kk[u]);
            r += nb;
        }
    }
...
print("### applied R10-%s (BATCH=%d RING=%d)  kernel md5 %s -> %s" % (MODE, BATCH, RING, before, md5(txt)))
